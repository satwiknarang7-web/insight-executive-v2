...
const INK = {
  dark: { text: '#c3c2b7', muted: '#8b8a84', grid: 'rgba(255,255,255,0.07)', surface: '#161615', cursor: 'rgba(255,255,255,0.05)' },
  light: { text: '#52514e', muted: '#7a7975', grid: 'rgba(0,0,0,0.07)', surface: '#ffffff', cursor: 'rgba(0,0,0,0.04)' },
};
...
function TooltipBox({ active, payload, label, fmtLabel, fmtValue, ink }) {
  ...
  return (
    <div className="rounded-lg border border-white/10 px-3 py-2 text-[12px] shadow-xl" style={{ background: ink.surface, color: ink.text, maxWidth: 280 }}>
      {label !== undefined && label !== null && <div className="mb-1 font-bold" style={{ color: ink.text }}>{fmtLabel ? fmtLabel(label) : label}</div>}
      {payload.map((p) => (
        <div key={`${p.dataKey}-${p.name}`} className="flex items-center gap-2">
          <span className="inline-block h-2 w-2 shrink-0 rounded-full" style={{ background: p.color || p.payload?.fill }} />
          <span className="min-w-0 flex-1 truncate">{p.name}</span>
          <span className="font-mono font-semibold">{fmtValue(p.value, p.dataKey, p)}</span>
        </div>
      ))}
    </div>
  );
...
export default function TileChart({ tile, measures = [], fields = [], height = 280, onSelect = null, selected = [] }) {
  ...
  const axis = { stroke: ink.grid, tick: { fill: ink.muted, fontSize: 11 }, tickLine: false, axisLine: false };
    ...
    return (
      <ResponsiveContainer width="100%" height={height}>
        <BarChart data={data} layout={horizontal ? 'vertical' : 'horizontal'} margin={{ top: 8, right: 16, bottom: 4, left: 4 }} barCategoryGap={tile.kind === 'distribution' ? 1 : '22%'}>
          <CartesianGrid stroke={ink.grid} vertical={!horizontal} horizontal={horizontal ? false : true} />
          {horizontal ? (
            <>
          ...
          <Tooltip cursor={{ fill: ink.cursor }} content={<TooltipBox ink={ink} fmtValue={(v, k, p) => valueFmt(v, k, p)} />} />
          <Bar dataKey={yKey} name={tile.kind === 'distribution' ? 'Rows' : 'Score'} fill={color(0)} radius={horizontal ? [0, 4, 4, 0] : [4, 4, 0, 0]} isAnimationActive={false} />
        </BarChart>
      </ResponsiveContainer>
    );
  }
        ...
        <ScatterChart margin={{ top: 8, right: 16, bottom: 18, left: 4 }}>
          ...
          <Tooltip
            ...
            content={({ active, payload }) => {
              ...
              return (
                <div className="rounded-lg border border-white/10 px-3 py-2 text-[12px] shadow-xl" style={{ background: ink.surface, color: ink.text }}>
          ...
          />
          {series.length > 1 && <Legend verticalAlign="top" height={24} iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, color: ink.text }} />}
          {series.map((s, i) => (
            <Scatter key={s.name} name={s.name} data={s.data} fill={color(i)} fillOpacity={0.7} stroke={ink.surface} strokeWidth={1} isAnimationActive={false} />
          ))}
        </ScatterChart>
      </ResponsiveContainer>
    );
      ...
      <div className="flex h-full w-full flex-col items-center gap-3 sm:flex-row" style={{ minHeight: height }}>
        <div className="h-[200px] w-full max-w-[220px] shrink-0">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={pieData} dataKey="value" nameKey="name" innerRadius="58%" outerRadius="92%" paddingAngle={1} stroke={ink.surface} strokeWidth={2} isAnimationActive={false} onClick={click ? (d) => onSelect(tile.dim, d.name) : undefined}>
                {pieData.map((d, i) => (
  ...
  const legend = multi ? <Legend verticalAlign="top" height={28} iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11, color: ink.text }} /> : null;
  const tooltip = <Tooltip cursor={{ fill: ink.cursor, stroke: ink.muted }} content={<TooltipBox ink={ink} fmtLabel={labelX} fmtValue={(v, k) => valueFmt(v, k)} />} />;
    ...
    return (
      <ResponsiveContainer width="100%" height={height}>
        <Chart data={data} margin={{ top: 8, right: 16, bottom: 4, left: 4 }}>
          ...
          {series.map((k, i) =>
            viz === 'line' ? (
              <Line key={k} type="monotone" dataKey={k} name={nameOf(k)} stroke={seriesColor(k, i)} strokeWidth={2} dot={data.length <= 24 ? { r: 2.5, strokeWidth: 0, fill: seriesColor(k, i) } : false} activeDot={{ r: 4 }} connectNulls isAnimationActive={false} />
            ) : (
              <Area key={k} type="monotone" dataKey={k} name={nameOf(k)} stroke={seriesColor(k, i)} strokeWidth={2} fill={seriesColor(k, i)} fillOpacity={viz === 'stackedArea' ? 0.55 : 0.14} stackId={viz === 'stackedArea' ? 's' : undefined} connectNulls isAnimationActive={false} />
            )
          )}
        </Chart>
      </ResponsiveContainer>
      ...
      <BarChart data={data} layout={horizontal ? 'vertical' : 'horizontal'} margin={margin} barCategoryGap="22%">
        ...
        {series.map((k, i) => (
          ...
          >
            {!multi &&
              data.map((d) => (
                <Cell key={String(d[c.x])} fill={d[c.x] === 'Other' ? ink.muted : seriesColor(k, i)} opacity={selected?.length && !isSelected(d[c.x]) ? 0.35 : 1} />
              ))}
          </Bar>
        ))}
      </BarChart>
  ...
  return (
    <div className="w-full overflow-auto" style={{ maxHeight: data.length <= 12 ? undefined : Math.max(height, 420) }}>
      <table className="w-full min-w-[420px] text-left text-[12px]">
        <thead className="sticky top-0 bg-[var(--card-bg)] text-[10px] uppercase tracking-[0.12em] text-white/45">
          <tr>
        ...
        <tbody>
          {data.map((r, i) => (
            <tr key={`${r[tile.dim]}-${i}`} className="border-t border-white/6">
              ...
              {ms.map((m, j) => (
                <td key={m.id} className="relative py-2 pl-3 text-right font-mono text-white/80">
                  {j === 0 && (
                    <span className="absolute inset-y-1.5 right-0 rounded-sm bg-accent-500/15" style={{ width: `${Math.round((Math.abs(r[m.id] || 0) / max) * 100)}%` }} />
                  )}
  ...
  return (
    <div className="w-full overflow-auto" style={{ maxHeight: Math.max(height, 380) }}>
      <table className="w-full border-separate border-spacing-[2px] text-[11px]">
        <thead>
          <tr>
            <th className="sticky left-0 bg-[var(--card-bg)] px-2 py-1.5 text-left font-bold text-white/45">{field(tile.dim)?.label || tile.dim}</th>
            {cols.map((k) => (
        ...
        <tbody>
          {data.map((r) => (
            <tr key={String(r[tile.dim])}>
              <td className="sticky left-0 max-w-[160px] truncate bg-[var(--card-bg)] px-2 py-1.5 font-semibold text-white/80" title={String(r[tile.dim])}>{String(r[tile.dim])}</td>
              {cols.map((k) => (
                <td key={k} className="rounded px-1 py-1.5 text-center font-mono" style={{ background: shade(r[k]), color: ink.text }} title={`${r[tile.dim]} · ${colLabel(k)}: ${formatValue(r[k], mFor(k))}`}>
