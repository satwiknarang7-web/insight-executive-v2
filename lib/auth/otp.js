...
/** The shortest password accepted. Stated once so the form and the route agree. */
export const MIN_PASSWORD = 8;

/** Minimum password rules, stated once so sign-up and reset cannot disagree. */
/**
 * Every reason a one-time code can be issued.
 *
 * Declared here, in the one auth module with no server dependencies, so the
 * routes, the mailer and the test that checks the database agrees can all read
 * the same list.
 *
 * The database has its own CHECK constraint on this column and that constraint
 * is the real gate — it is what stops a caller inventing a purpose the verify
 * route has no branch for. The two drifted apart once: `recover` was added to
 * the code and not to the constraint, every reset failed inside a `catch` whose
 * job is to say nothing, and the flow looked like it worked. `tests/challengePurposes`
 * compares this list against the migrations so that cannot happen twice.
 */
export const CHALLENGE_PURPOSES = ['signup', 'signin', 'recover'];
