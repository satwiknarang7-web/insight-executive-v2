...
export async function sendCodeEmail({ to, code, purpose }) {
  ...
  // Three things a code can be for, and the email says which. A message that
  // read "your sign-in code" next to a password reset would be the one piece of
  // mail a reader most needs to recognise, arriving disguised as routine.
  const HEADINGS = {
    signup: ['Confirm your email', 'Use this code to finish creating your Insight Executive account.'],
    recover: [
      'Reset your password',
      'Use this code to set a new password for your Insight Executive account. Your current password still works until you do.',
    ],
  };
  const [heading, lead] = HEADINGS[purpose] || [
    'Your sign-in code',
