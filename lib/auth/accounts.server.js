...
import { findUserInPages } from './lookup';
 ...
 *
 * The admin API has no get-by-email and no server-side filter — `listUsers`
 * only paginates — so the pages are walked (see ./lookup). Reading just the
 * first one, as this used to, meant the lookup silently started failing at the
 * 201st account: sign-up would tell someone their unfinished account was
 * already registered and refuse to send another code, and sign-in's recovery
 * path for an unconfirmed address would answer a correct password with "does
 * not match".
 */
export async function findUserByEmail(email, admin = null) {
  return findUserInPages(admin || serviceClient().auth.admin, email);
}

/**
 * The account behind a user id.
 *
 * Used to find where to send a "this was shared with you" notification. The
 * caller must already have established that it is entitled to act on that user
 * — this looks up an address with the service role and does no authorisation of
 * its own. Returns null rather than throwing, because a missing recipient is a
 * reason not to send an email, not a reason to fail the share that succeeded.
 */
...
/**
 * Set a new password, once a code has proved the inbox is theirs.
 *
 * The only way a password changes without the old one, and it is reachable in
 * exactly one place: the recovery branch of `/api/auth/verify`, after a
 * challenge raised for that purpose has been claimed. The code is the proof —
 * whoever reads the mailbox owns the account — which is the same evidence
 * sign-up accepts to confirm an address in the first place.
 */
