 ...
 *
 * This is worth its own type because it is the one failure here that is a
 * deployment step rather than a bug, and it is otherwise deeply confusing: the
 * code is stored before it is emailed, so a missing table presents to the user
 * as "the email never arrived" and sends everyone hunting through SMTP settings
 * that are working perfectly.
 */
export class AuthSchemaMissing extends Error {
  constructor() {
    super(
      'The sign-in tables are missing from this Supabase project. ' +
        'Run supabase/APPLY_TO_LIVE_PROJECT.sql in its SQL editor.'
    );
