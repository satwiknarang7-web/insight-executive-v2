import 'server-only';
import { CHALLENGE_PURPOSES } from './otp.js';
...
export async function createChallenge({ email, userId = null, purpose }) {
  // Checked here so a bad purpose fails as a bad purpose, loudly and at the
  // call site, rather than as an opaque constraint violation three layers down
  // inside a catch that exists to reveal nothing.
  if (!CHALLENGE_PURPOSES.includes(purpose)) {
