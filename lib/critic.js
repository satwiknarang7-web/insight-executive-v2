...
/**
 * How many model questions are worth reading before they become wallpaper.
 * The deterministic ones are found, not generated, so they keep their places
 ...
 *
 * Column names, what kind of column each is, how many levels it has, and the
 * titles of the charts already built. No values, no metrics, not one figure
 * from the analysis.
 *
 * That is not only a privacy decision. It makes the guarantee structural: a
 * model with no numbers in front of it has none to quote, so any digit in its
 * reply is invented and can be rejected mechanically rather than argued with
 * in a prompt. `acceptModelQuestions` does exactly that.
 */
export function modelBriefing({ profile = null, findings = [], questions = [] } = {}) {
  const cardinality = profile?.cardinality || {};
  const columns = [
    ...(profile?.dimensions || []).map((name) => ({ name, kind: 'category', levels: cardinality[name] || null })),
    ...(profile?.measures || []).map((name) => ({ name, kind: 'number', levels: cardinality[name] || null })),
  ];
  return {
    columns,
    charted: (findings || []).map((f) => f?.title).filter(Boolean),
    alreadyAsked: (questions || []).map((q) => q?.question).filter(Boolean),
  };
}

/**
 * Every digit run in a string that is not part of one of these names.
 *
 * Exported because the analyst agent needs the same test on the titles it
 * writes, and for the same reason: it is shown no values, so a figure in its
 * output came from nowhere.
 */
export function inventedNumber(text, names) {
...
/**
 * Take what the model returned, and keep only what it was allowed to say.
 *
 * The rules are the same ones the deterministic critic obeys, enforced here
 * because the author is not this codebase. A question, and only a question.
 * Nothing that quotes a figure, because it was shown none — so a digit outside
 * a column name is an invention, and that is checkable rather than debatable.
 * Nothing already asked, so the list does not say the same thing twice in two
 * voices. And every one of them marked as model-written, because a reader has
 * to be able to tell which questions were found in the data and which were
 * thought up about it.
 *
 * @param {unknown}  raw       whatever the route parsed out of the reply
 * @param {object}   context
 * @param {string[]} context.columns   column names, for the digit test
 * @param {object[]} context.existing  questions the deterministic pass produced
 */
export function acceptModelQuestions(raw, { columns = [], existing = [] } = {}) {
  ...
  for (const item of raw) {
    ...
    const question = String(text ?? '').trim().replace(/\s+/g, ' ');
    if (!question || question.length > MAX_QUESTION_LENGTH) continue;
    // A question, not an assertion dressed as one.
    if (!question.endsWith('?')) continue;
    if (inventedNumber(question, columns)) continue;
    const key = normalise(question);
    if (!key || seen.has(key)) continue;
    seen.add(key);
    out.push({ kind: 'model', source: 'model', question, evidence: [] });
