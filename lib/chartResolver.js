...
export function resolveChart(rows, requested = {}) {
  ...
  if (type === 'scatter') {
    ...
    // A declared series is the lines, never the axis: years along the bottom
    // and a line per country, not countries along the bottom.
    if (numeric.includes(xKey)) {
      const cat = [...dimensions, ...p.categorical].find((k) => k !== requested.seriesKey);
      if (cat) xKey = cat;
    }
  }
