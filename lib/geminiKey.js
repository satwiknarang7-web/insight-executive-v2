 ...
 *
 * This used to be one regex — `AIza` and 35 more characters — written as if
 * that were *the* shape Google issues. It was the shape Google issued at the
 * time. AI Studio now also hands out keys beginning `AQ.`, and the panel
 * refused them with a confident sentence explaining that they were a key for
 * something else. They were not; the sentence was.
 *
 * So the check is now the other way round. These patterns exist to give a
 * better message for the mistakes that are actually common, not to decide what
 * a valid key is. That decision belongs to Google, `verifyKey` already asks it
 * before anything is saved, and a shape nobody here has seen before is allowed
 * through to be asked about rather than turned away on a guess.
 */

/** The classic key: `AIza` and 35 more characters of URL-safe base64. */
...
/**
 * What is accepted at all: a plausible credential and nothing that could be
 * anything else.
 *
 ...
 * dialled out to Google, and that is the only job it has.
 */
export const KEY_PATTERN = /^[0-9A-Za-z._-]{20,200}$/;

/** Prefixes belonging to providers that are definitely not Google. */
 ...
 *
 * Says nothing it cannot support. A key of a shape this file does not
 * recognise gets no complaint at all — it goes to Google, which is the only
...
export function keyProblem(key) {
  ...
  for (const [prefix, provider] of OTHER_PROVIDERS) {
    if (value.startsWith(prefix)) return `That looks like an ${provider} key, not a Google one.`;
  }

  // The two shapes we know, and the mistake each of them fails on: a copy that
  // stopped short. Length is worth naming because it is fixable by the person
  // reading the message.
  if (value.startsWith('AIza') && !CLASSIC_PATTERN.test(value)) {
    if (value.length !== 39) {
      return `A key starting "AIza" is 39 characters; that one is ${value.length}. It was probably copied short.`;
    }
    return 'That has characters a Google key does not use.';
  }
  if (value.startsWith('AQ.') && !STUDIO_PATTERN.test(value)) {
    return 'That looks like an AI Studio key that was cut short. Copy the whole thing.';
  }
