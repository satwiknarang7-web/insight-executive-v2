...
/**
 * Add or remove one value, keeping the rest.
 *
 * What a checkbox does, as against what a bar does: a slicer with three boxes
 * ticked is asking for all three, and unticking the last one leaves the column
 * unfiltered rather than filtered to nothing.
 */
export function toggleInSet(list = [], column, value) {
  const text = String(value);
  const existing = list.find((f) => f.column === column && f.kind === VALUES);
  if (!existing) return [...list, { kind: VALUES, column, values: [text] }];

  const values = existing.values.includes(text)
    ? existing.values.filter((v) => v !== text)
    : [...existing.values, text];
  if (!values.length) return list.filter((f) => f !== existing);
  return list.map((f) => (f === existing ? { ...f, values } : f));
}

/** Every value of a column currently kept, for a control that shows its state. */
export function selectedValues(list = [], column) {
  return list.find((f) => f.column === column && f.kind === VALUES)?.values || [];
}
...
export function clickTarget(chart, { columns = [], temporal = [] } = {}) {
  // A filter tile is the one chart whose whole purpose is this, and it ticks
  // rather than picks: a second box is a second value kept, not a change of
  // mind about the first. Its axis IS the column, by construction.
  if (chart?.chart_type === 'slicer') {
    return columns.includes(chart.xAxisKey) ? { column: chart.xAxisKey, kind: VALUES, multi: true } : null;
  }
...
export function applyClick(list = [], target, value) {
  ...
  if (target.kind === VALUES) {
    return target.multi
      ? toggleInSet(list, target.column, value)
      : toggleValue(list, target.column, value);
  }
