...
export function createTdsDriver({ id, dialect = 'sqlserver', configFor, hostFor }) {
  ...
  async function listTables(config) {
    return withPool(config, async (pool) => {
      /**
       * Catalog names in capitals, deliberately.
       *
       * SQL Server resolves object and column names using the *database's*
       * collation, and a Fabric warehouse is created with
       * `Latin1_General_100_BIN2_UTF8` — binary, and therefore case-sensitive.
       * The catalog views are declared as `INFORMATION_SCHEMA.TABLES` with
       * `TABLE_SCHEMA` and friends, so the lowercase spelling that works
       * everywhere else came back as "Invalid object name
       * 'information_schema.tables'" against Fabric, immediately after a
       * connection that had just succeeded.
       *
       * Uppercase is correct on both: a case-insensitive server does not care,
       * and a case-sensitive one requires it. The aliases stay lowercase
       * because they are ours and are read back by this code.
       */
      const result = await pool.request().query(
        `select top 2000
