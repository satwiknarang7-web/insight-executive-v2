...
/**
 * Write verbs, matched only where a statement can actually begin.
 *
 * By the time this runs the query has already had to start with SELECT or WITH
 * and contain no semicolon, so the only place another statement can hide is
 * inside a CTE body — `WITH x AS (DELETE ... RETURNING *)`. Matching the verb
 * after an opening parenthesis catches that precisely.
 *
 * The previous version matched these words anywhere in the query, which meant
 * `SELECT comment FROM posts` was rejected: `comment` is a perfectly ordinary
 * column name, as are `set`, `lock`, `call` and `do`. Blocking real queries to
 * re-block writes that the leading-keyword check already refuses is a bad trade.
 */
const WRITE_IN_STATEMENT_POSITION = /\(\s*(insert|update|delete|merge|replace|upsert)\b/i;

/**
 * Tokens that are never a column name and never belong in a read query. Every
 * entry is either multi-word or a vendor procedure, so none of them can collide
 * with a real identifier.
 */
const NEVER_LEGITIMATE =
  /\b(drop\s+(table|database|schema|view|index)|truncate\s+table|alter\s+(table|database|schema)|create\s+(table|database|schema|view|index)|grant\s+\w|revoke\s+\w|xp_cmdshell|sp_executesql)\b/i;

/**
 * Row-locking clauses. Harmless to the data, but they take locks on what is
 * usually a production database, and an import has no business doing that.
 * Anchored on `for` so a column called `update` or `share` still works.
 */
const LOCKING_CLAUSE = /\bfor\s+(update|share|no\s+key\s+update|key\s+share)\b/i;

/**
 * Accept exactly one read-only statement, and return it ready to run.
 *
 ...
 *
 * Two texts matter here and confusing them is a bug that reaches production:
 *
 * - `bare` has comments and quoted text removed. Every CHECK runs against it,
 *   so a `;` or a `DROP` hidden inside a string literal cannot fool them.
 * - `raw` is what the caller wrote. It is what gets RETURNED and executed.
 *
 * Returning `bare` was the original mistake. Drivers execute this return value,
 * and stripping quoted text destroys the statement: Postgres identifiers are
 * double-quoted, so `select * from "public"."orders"` came back as
 * `select * from  .` — a syntax error at the first table anyone imported. Worse,
 * a query that survived parsing came back with its filters gutted, turning
 * `where region = 'West'` into `where region =`.
 */
export function assertReadOnlySql(sql) {
  ...
  const writableCte = WRITE_IN_STATEMENT_POSITION.exec(bare);
  if (writableCte) {
    throw new UnsafeQuery(
      `"${writableCte[1].toUpperCase()}" is not allowed here — this connection is read-only.`
    );
  }
  const ddl = NEVER_LEGITIMATE.exec(bare);
  if (ddl) {
    throw new UnsafeQuery(
      `"${ddl[0].replace(/\s+/g, ' ').toUpperCase()}" is not allowed here — this connection is read-only.`
    );
  }
  if (LOCKING_CLAUSE.test(bare)) {
    throw new UnsafeQuery('Row-locking clauses such as FOR UPDATE are not allowed on an imported query.');
  }
  // The writable-CTE check above catches `WITH x AS (INSERT ... RETURNING)`;
  // this catches the SELECT INTO form, which creates a table.
  ...
  // The caller's own text, minus a terminating semicolon so it can be wrapped
  // in a bounding subquery. Any semicolon outside quoted text was already
  // rejected above, so a trailing one here is a terminator and nothing else.
