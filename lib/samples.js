...
/**
 * A file with the mess real exports actually arrive with.
 *
 * The other three samples are clean, which makes them useless for showing the
 * one thing this tool does that others do not: say out loud which cells it had
 * to guess at, and refuse to let a finding claim more than those cells support.
 * On a clean file that machinery is invisible, so nobody could see it working —
 * including the person who built it, who had to generate a file by hand every
 * time they wanted to look at it.
 *
 * Every fault here is one that happens constantly and reads as harmless:
 *
 *   - dates written `05/04/2025`, where day-first and month-first both parse
 *     and disagree by a month
 *   - an amount column of `1,234` with nothing in it proving whether the comma
 *     groups digits or divides them — a hundredfold difference either way
 *   - a scattering of rows that end early, so trailing cells read as blank
 *     when they were never written at all
 *
 * The numbers underneath are real enough to analyse: West leads, Electronics
 * carries the revenue, and the trend rises. The point is that the deck says so
 * more quietly than it does for `retail`, and says why.
 */
function messyCsv() {
  const r = rng(20260913);
  const regions = ['North', 'South', 'East', 'West', 'Central'];
  const categories = ['Electronics', 'Apparel', 'Home & Garden', 'Grocery'];
  const reps = ['A. Mensah', 'J. Park', 'L. Okafor', 'R. Silva', 'T. Nguyen'];
  const regionLift = { North: 1.1, South: 0.85, East: 1.0, West: 1.3, Central: 0.75 };
  const catPrice = { Electronics: 380, Apparel: 70, 'Home & Garden': 140, Grocery: 32 };

  const rows = [];
  for (let i = 0; i < 480; i++) {
    const day = new Date(Date.UTC(2025, 0, 1 + Math.floor(i * 0.75)));
    const region = pick(r, regions);
    const category = pick(r, categories);
    const units = 1 + Math.floor(r() * 12);
    const price = catPrice[category] * (0.85 + r() * 0.3) * regionLift[region];
    const revenue = round(units * price);

    // Both parts twelve or less, so the reading is a coin toss and the other
    // one lands in a different month. Written day-first, read month-first.
    const dd = String(day.getUTCDate()).padStart(2, '0');
    const mm = String(day.getUTCMonth() + 1).padStart(2, '0');
    const ambiguous = day.getUTCDate() <= 12;
    const ordered = ambiguous ? `${dd}/${mm}/${day.getUTCFullYear()}` : day.toISOString().slice(0, 10);

    // No value anywhere in this column carries a decimal point, so nothing
    // proves the convention and the whole column is read on an assumption.
    const grouped = `${Math.floor(revenue).toLocaleString('en-US')}`;

    const row = [ordered, region, category, pick(r, reps), units, grouped];
    // One row in fourteen stops early, the way a stray unquoted comma or a
    // half-finished export leaves them.
    if (i % 14 === 13) row.length = 4;
    rows.push(row);
  }

  return toCsv(['ordered_on', 'region', 'category', 'rep', 'units', 'revenue'], rows);
}
...
export const SAMPLES = [
  ...
  },
  {
    key: 'messy',
    title: 'Messy export',
    description: '480 orders with ambiguous dates, unprovable commas and short rows',
    get csv() {
      return messyCsv();
    },
  },
