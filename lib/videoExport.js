 ...
 * slideshow does. When a slide has narration its audio plays under it, and
 * its duration is the narration's length plus a breath.
 *
 * It records with MediaRecorder from a canvas and a Web Audio destination,
 * which means recording runs in real time: a two-minute deck takes two minutes
 * to record. That is the price of needing no server, no ffmpeg and no upload —
 * the slides never leave the browser. The audio clock is the master clock, so
 * the pictures stay in step with the voice even if frames are dropped.
 */
 ...
 * Record the slides.
 *
 * slides:     [{ image: CanvasImageSource, audio: AudioBuffer | null, ms: number }]
 * audioCtx:   an AudioContext, created in the click that started the export so
 *             the browser lets it make sound.
 ...
 */
export async function recordSlides({ slides, audioCtx, width = 1920, height = 1080, background = '#0a0a0b', accent = '#34d399', onProgress, signal }) {
  ...
  const hasAudio = !!audioCtx && slides.some((s) => s.audio);
  ...
  const recorder = new MediaRecorder(new MediaStream(tracks), { mimeType: type, videoBitsPerSecond: 6_000_000, audioBitsPerSecond: 128_000 });
  const chunks = [];
  ...
  // One clock for both: the audio context's when there is sound, the page's
  // when there is not.
  const clockStart = hasAudio ? audioCtx.currentTime : 0;
  const pageStart = performance.now();
  const now = () => (hasAudio ? (audioCtx.currentTime - clockStart) * 1000 : performance.now() - pageStart);
  ...
  if (hasAudio) {
    slides.forEach((s, i) => {
      ...
      src.buffer = s.audio;
      src.connect(dest);
      // A short beat after the slide lands, then the voice.
      src.start(clockStart + (starts[i] + 350) / 1000);
      sources.push(src);
    });
  }
  ...
  // Let the last frame and the tail of the audio reach the recorder.
