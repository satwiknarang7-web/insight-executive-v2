 ...
 *
 * Eight was enough for the two questions this briefing used to serve — which
 * columns share a unit, which values mean a row did not stand — because both
 * are answered from a column's vocabulary and the rows were context. Naming the
 * dataset's *subject* is a different question: it is answered by seeing what a
 * whole record looks like, and eight records taken at a stride through a
 * 3,000-row file can miss an entire regime of it.
 *
 * Twenty is the ceiling the user agreed to send. It stays a stride sample
 * rather than the head of the file, so a table sorted by its own key does not
 * present twenty rows of one category as the whole dataset.
 */
...
export function valueVocabulary(rows = [], profile = null, { maxLevels = MAX_LEVELS } = {}) {
  ...
  // A column whose cardinality is unknown is EXCLUDED, not included.
  //
  // This read `!cardinality[d] || cardinality[d] <= maxLevels`, which fails
  // open: a missing entry admitted the column, so a table where the profile had
  // not counted a column built a value map over every distinct customer name,
  // email and postcode in it — the exact columns the cap exists to keep out,
  // held in memory and sent nowhere useful. The safe direction for "I do not
  // know how many values this has" is to leave it out.
  const dimensions = (profile?.dimensions || []).filter(
    (d) => Number.isFinite(cardinality[d]) && cardinality[d] <= maxLevels
  );
