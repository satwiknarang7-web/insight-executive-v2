 ...
 *
 * The table model (lib/tableModel.js) decides which charts are *valid* for a
 * schema: which columns can be summed, which can be averaged, which are
 * categories worth grouping by. That is a question about column shapes.
 *
 ...
 * nothing to say" and 1 means "this is the finding", so a chart earns its place
 * by what it shows rather than by where it sat in a fixed playbook.
 *
...
export function trendStrength(values) {
  ...
  const scale = Math.abs(my) || ys.reduce((m, y) => Math.max(m, Math.abs(y)), 0) || 1;
