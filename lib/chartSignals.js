 ...
 */
export function outcomeSpread(groups, { minGroup = 12 } = {}) {
  const usable = groups.filter((g) => g.n >= minGroup && isNum(g.rate));
  ...
  let z = 0;
  for (const g of usable) {
    const se = Math.sqrt((base * (1 - base)) / g.n);
    if (se > 0) z = Math.max(z, Math.abs(g.rate - base) / se);
  }
  const confidence = unit((z - 2) / 2);

  return unit((spread / base) * confidence);
}

/**
 * One group per level of `dimension`, carrying the share of rows in it where
 * `column` holds `event`.
 */
export function outcomeGroups(rows, dimension, column, event) {
  const buckets = new Map();
  ...
  return [...buckets.values()]
    .map((b) => ({ ...b, rate: b.n ? b.hits / b.n : null, value: b.n ? b.hits / b.n : 0 }))
