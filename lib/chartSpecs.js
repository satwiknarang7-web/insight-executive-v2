...
const REQUIREMENTS = {
  ...
  funnel: ranked('funnel', 'Stages that shrink. Order the categories the way the process runs.', {
    dimensions: [dim('dimension', 'Stage')],
    limit: SPOKE_N,
  }),
  waterfall: ranked(
    'waterfall',
    'How a running total is built up. Needs an ordered x axis — a period, or a sequence of steps.',
    { dimensions: [dim('dimension', 'Step', { prefer: 'time' })], ordered: true }
  ),
  line: ranked('line', 'A measure along an ordered axis. Pick a date or period column.', {
    dimensions: [dim('dimension', 'Period', { prefer: 'time' })],
    ordered: true,
  }),
...
export function chartArity(type) {
  ...
  return { dimensions: req.dimensions.length, measures: req.measures.length };
}
 ...
 */
function orderClause(req, dimCols, aliases) {
  if (req.ordered && dimCols.length) {
    const secondary = dimCols.length > 1 ? `, [${aliases[0]}] DESC` : '';
    return ` ORDER BY [${dimCols[0]}] ASC${secondary}`;
  }
  if (!aliases.length) return '';
  return ` ORDER BY [${aliases[0]}] DESC`;
}
 ...
 */
export function buildChartSpec({ type = 'bar', dims = {}, vals = {}, limit = null } = {}, context = {}) {
  const req = chartRequirement(type);
  ...
  for (const slot of req.dimensions) {
    ...
    if (dimCols.includes(column)) {
      return fail('A chart with two categories needs two different columns.');
    }
    dimCols.push(column);
  }
  ...
  const groupBy = dimCols.length ? ` GROUP BY ${dimCols.map((c) => `[${c}]`).join(', ')}` : '';
  const order = dimCols.length ? orderClause(req, dimCols, aliases) : '';
  const rows = Number(limit) || req.limit?.preset || 0;
  const limitClause = req.limit && rows > 0 ? ` LIMIT ${rows}` : '';
  const columns = [...dimCols.map((c) => `[${c}]`), ...selects].join(', ');
