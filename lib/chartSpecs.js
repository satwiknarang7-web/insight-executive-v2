...
import { MAX_SERIES } from './chartPalette.js';
...
// A filter tile is a list to pick from, so it wants every value it can show
// rather than a top ten — but a column with a thousand of them is a scrollbar
// with no end, and a filter nobody can use.
const VALUES_N = { label: 'Offer up to', options: [10, 25, 50, 100, 250], preset: 50 };
...
// A legend is only honest while the eye can still tell the series apart, which
// is however many colours the palette can field without repeating one.
...
const REQUIREMENTS = {
   ...
   * A filter, as a chart.
   *
   * Power BI calls it a slicer, and it belongs in this list rather than in a
   * toolbar: it is a tile on the grid, it is about one column, it is sized and
   * moved like everything else on the board, and — the part that makes it a
   * chart rather than a control — it runs a query. Its rows are the column's
   * values with the number of rows behind each, so the counts beside the boxes
   * are computed the same way every other figure here is, and narrow with the
   * rest of the deck when another filter is on.
   *
   * No measure to choose: what a slicer measures is how many rows there are,
   * always. The `fixed` slot is how that reaches the SQL without asking.
   */
  slicer: {
    label: 'slicer',
    blurb:
      "A list of one column's values with a box beside each. Ticking them filters every chart on the dashboard.",
    dimensions: [dim('dimension', 'Filter on', { help: 'The column whose values are offered.' })],
    measures: [val('measure', 'Rows', { fixed: 'COUNT' })],
    limit: VALUES_N,
    sortable: true,
  },
...
/**
 * How a slicer offers its values.
 *
 * Both are the same list with the same boxes; the difference is whether it is
 * open. A list is faster to read and to tick, and a dropdown is how a filter
 * fits on a board with no room to spare — which is a judgement about the
 * arrangement, so it belongs to whoever arranged it rather than to a rule here.
 */
export const SLICER_MODES = {
  list: { label: 'List', blurb: 'Every value on show, with a box beside each.' },
  dropdown: { label: 'Dropdown', blurb: 'One line that opens the list, for a tile with little room.' },
};

export const DEFAULT_SLICER_MODE = 'list';

export const isSlicerMode = (mode) => Object.prototype.hasOwnProperty.call(SLICER_MODES, mode);
...
export function buildChartSpec(
  ...
  for (const slot of req.measures) {
    // A fixed slot is not a choice. The slicer counts rows, and asking which
    // column to count would be asking a question with one answer.
    const built = measureExpression(slot.fixed ? { aggregate: slot.fixed, column: '' } : vals[slot.key], context);
