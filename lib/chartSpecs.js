 ...
 */
function orderClause(req, dimCols, aliases, sort) {
  ...
  if (chosen.startsWith('category') && dimCols.length) {
    return ` ORDER BY [${dimCols[0]}] ${chosen === 'category-asc' ? 'ASC' : 'DESC'}`;
  }
  if (!aliases.length) return '';
  return ` ORDER BY [${aliases[0]}] ${chosen === 'value-asc' ? 'ASC' : 'DESC'}`;
}
...
export function buildChartSpec(
  ...
  for (const slot of req.dimensions) {
    ...
    const grouped = slot.prefer === 'time' || req.ordered ? bucketExpression(column, bucket, context) : null;
    if (grouped) {
      bucketed = grouped;
  ...
  const groupBy = dimSelects.length
    ...
    : '';
  const order = dimCols.length ? orderClause(req, dimCols, aliases, sort) : '';
