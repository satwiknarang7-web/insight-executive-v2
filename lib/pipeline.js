...
import { resolveChart, isAggregatedSql, ADVANCED_TYPES, profileColumns, isTemporalValue } from './chartResolver.js';
import { analyzeStoryboard } from './insightEngine.js';
...
/** Run one SQL string against the mounted table, normalising table references. */
export function runSql(sql, table = TABLE) {
  const cleaned = String(sql || '')
    .replace(/\[SalesData\]/gi, table)
    .replace(/SalesData/gi, table);
...
function compositeLabels(resultData, chartType, requestedXKey) {
  ...
  if (stringKeys.length <= 1 || wantsScatter) return { resultData, xKey: requestedXKey };
 ...
 */
export function runAnalysis(rows, { focus = null, maxCharts = null, onProgress } = {}) {
  ...
  const specs = planCharts(rows, { max: target });
