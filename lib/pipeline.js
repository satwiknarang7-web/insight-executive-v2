...
export function runAnalysis(
  ...
  const specs = planCharts(rows, { max: target, provenance, roles, claims, profile: shape, repeated });
  const kpis = planKpis(rows, { provenance, roles, claims, profile: shape, repeated });
  ...
  const board = composeBoard(charts, filters);
  for (const chart of [...charts, ...filters]) {
    const box = board.get(String(chart.id));
    if (box) chart.layout = box;
  }
