...
import { nearDuplicates, contestedBasis, argumentSpine, spineAsBullets } from './synthesiser.js';
...
export function runAnalysis(
  rows,
  { focus = null, maxCharts = null, tables = null, provenance = {}, roles = {}, profile = null, withheld = [], claims = null, onProgress } = {}
) {
  ...
  const specs = planCharts(rows, { max: target, provenance, roles, claims });
  const kpis = planKpis(rows, { provenance, roles, claims });
  ...
  const shape = profile || profileColumns(rows);
   ...
   */
  const contested = contestedBasis(rows, shape);
