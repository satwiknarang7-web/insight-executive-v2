...
export const INGEST_FILES = [
  ...
  // Run by the provider once the worker is done: the columns the table should
  // have had (a month, a split field, a band), added before any chart is made.
  { id: 'prepare', label: 'Prepare the data', stages: ['Preparing the data'] },
...
export const INGEST_REMOTE = [
  // A document is read by a model before there are any rows to clean. Rows
  // from a database arrive already read, so this step simply never fires for
  // them — one list, because everything after the first step is identical.
  { id: 'read', label: 'Read the document', stages: ['Reading the document'] },
...
export const ANALYZE = [
  /*
   * What an analyst does before the first chart: reads the table and adds the
   * columns it should have had. Runs in the provider, once per dataset, and
   * only when a model is reachable — so on most re-runs this step is simply
   * passed, which the panel shows as done rather than skipped.
   */
  { id: 'prepare', label: 'Prepare the data', stages: ['Preparing the data'] },
  ...
  /*
   * The deck is read before it is handed over.
   *
   * These two steps used to happen silently or not at all: the review ran after
   * the analysis was already on screen, and the report was assembled inside the
   * verify step. Naming them is not decoration — a reader watching the panel is
   * being told that something checked the work, and which of the stages the
   * slow one is.
   */
  { id: 'review', label: 'Review what it says', stages: ['Reviewing the deck'] },
  { id: 'report', label: 'Write the report', stages: ['Writing the report'] },
  /*
   * Sizing the cards and choosing the filters is work, and it is the last work
   * the engine does — a step rather than a silent pause between the report
   * being written and the deck appearing arranged.
   */
  { id: 'board', label: 'Arrange the board', stages: ['Arranging the board'] },
  /*
   * And then read it again, as a reader rather than as a statistician.
   *
   * These two run in the provider rather than the worker, because the second
   * one can call a model and the worker cannot. They are on the critical path
   * on purpose: a defect the customer has already seen is not worth finding.
   */
  { id: 'polish', label: 'Check how it reads', stages: ['Checking how it reads'] },
  { id: 'fix', label: 'Apply the fixes', stages: ['Applying the fixes'] },
  { id: 'ready', label: 'Ready', stages: ['Ready'] },
];
