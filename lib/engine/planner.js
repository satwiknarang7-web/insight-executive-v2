...
import { readDataset, toNumber } from './fields.js';
import { buildMeasures } from './measures.js';
import { binLabel, runQuery, fieldPairs } from './query.js';
import { allowedViz, chooseGrain, computeTile, MAP_VIZ, MAX_BARS } from './tiles.js';
...
function entityDims(ds) {
  return ds.fields.filter((f) => !f.alias && (f.role === 'dimension' || (f.role === 'id' && !(ds.key?.length === 1 && ds.key[0] === f.name))) && f.stats.distinct > 12 && f.stats.distinct <= 5000 && f.stats.fill >= 0.5);
}
...
function kpiTiles(rows, ds, measures, hints, primary) {
  ...
  const grain = time ? seriesGrain(time, chooseGrain(time), rows.length, 1) : null;
  const regular = uniformCounts(rows, ds, time ? chooseGrain(time) : null);
...
function candidates(rows, ds, measures, hints, primary) {
  ...
  const breakdownViz = (tile, m, computed) => {
    ...
    // Places, several of them: where they are is part of the story.
    if (allowed.includes('map') && (ds.byName[tile.dim]?.stats.distinct || 0) >= 5) return 'map';
    if (allowed.includes('donut') && !neg && vals.length <= 5 && topShare < 0.75) return 'donut';
    return allowed.filter((v) => v !== 'donut' && v !== 'map')[0];
  };
  ...
  if (time && grain && !ds.long) {
    ...
    if (points >= 4) {
      ...
      if (splitDim && primary && (primary.additive || primary.type === 'rate' || primary.type === 'agg')) {
        // Several lines at a fine grain cross and spike into a tangle; each line
        // needs enough rows per point to show a trend rather than noise.
        const sgrain = seriesGrain(time, grain, rows.length, splitDim.stats.distinct);
        const tile = { id: nextId('t'), kind: 'trend', measures: [primary.id], dim: time.name, grain: sgrain, series: splitDim.name, title: `${primary.label} by ${lower(splitDim.label)} over time`, section: 'trend', viz: 'line' };
        push({ ...make(rows, ds, measures, tile, (c) => seriesTrendInsight(c, primary, { grain: sgrain, seriesLabel: splitDim.label })), weight: 0.9, role: 'trend', measure: primary.id, pair: `${splitDim.name}@t` });
      }
    }
  }
  ...
  // 7b. Two splits that only make sense together: a heatmap, when the cells
  // depart from what each split predicts on its own by more than noise.
  {
    const perRow = (m) => {
      if (m.type === 'rate') return (r) => (r?.[m.event.field] == null || r?.[m.event.field] === '' ? null : String(r[m.event.field]) === String(m.event.value) ? 1 : 0);
      if (m.type === 'ratio' && m.den?.type === 'count' && m.num?.type === 'agg' && m.num.agg === 'sum' && !m.num.where?.length) return (r) => toNumber(r?.[m.num.field]);
      if (m.type === 'agg' && (m.agg === 'avg' || m.agg === 'median') && !m.where?.length) return (r) => toNumber(r?.[m.field]);
      return null;
    };
    const cellDims = dims.filter((d) => !d.alias && d.stats.distinct >= 2 && d.stats.distinct <= 10 && d.stats.fill >= 0.8).slice(0, 6);
    const typical = usable.filter((m) => perRow(m) && !(m.type === 'rate' && cellDims.some((d) => d.name === m.event.field))).slice(0, 2);
    const found = [];
    for (const m of typical) {
      const valueOf = perRow(m);
      for (let i = 0; i < cellDims.length; i++) {
        for (let j = i + 1; j < cellDims.length; j++) {
          const a = cellDims[i];
          const b = cellDims[j];
          if (m.type === 'rate' && (m.event.field === a.name || m.event.field === b.name)) continue;
          const res = interaction(rows, a.name, b.name, valueOf);
          if (res) found.push({ m, a, b, ...res });
        }
      }
    }
    ...
    const best = found[0];
    if (best) {
      const { m, a, b, cell } = best;
      const tile = { id: nextId('x'), kind: 'breakdown', measures: [m.id], dim: a.name, series: b.name, title: `${m.label} by ${lower(a.label)} and ${lower(b.label)}`, section: 'compare', viz: 'heatmap' };
      const made = make(rows, ds, measures, tile, () => ({
        text: `${a.label} and ${lower(b.label)} work together: ${cell.a} with ${cell.b} averages ${formatValue(cell.actual, m)}, against ${formatValue(cell.expected, m)} expected from each on its own.`,
        score: Math.min(0.85, best.strength * 2.5),
        facts: { actual: formatValue(cell.actual, m), expected: formatValue(cell.expected, m) },
      }));
      push({ ...made, weight: 0.9, role: 'matrix', measure: m.id, pair: `${a.name}×${b.name}` });
    }
  }
  ...
  // A top-10 table reads the same with fifty customers or fifty thousand, so
  // the cap on how many a chart can show does not apply here.
  const entity = ds.shape === 'entities' || ds.shape === 'survey' ? nameColumn(ds) : entityDims(ds)[0] || ds.fields.find((f) => (f.role === 'dimension' || (f.role === 'id' && !(ds.key?.length === 1 && ds.key[0] === f.name))) && f.stats.distinct > 5000 && f.stats.fill >= 0.5);
...
/**
 * How much two splits interact for a per-row value. Cell means are compared
 * with what the row and column means predict (additive model); the gap must be
 * at least a tenth of the overall level and three times the cells' own
 * sampling noise, with every cell backed by 30+ rows. Null when it is not.
 */
function interaction(rows, fa, fb, valueOf) {
  const cells = new Map();
  for (const r of rows) {
    const x = r?.[fa];
    const y = r?.[fb];
    ...
    const k = `${x}\u0000${y}`;
    let c = cells.get(k);
    if (!c) cells.set(k, (c = { a: String(x), b: String(y), n: 0, s: 0, ss: 0 }));
    c.n++;
    c.s += v;
    c.ss += v * v;
  }
  const list = [...cells.values()];
  const as = [...new Set(list.map((c) => c.a))];
  const bs = [...new Set(list.map((c) => c.b))];
  if (as.length < 2 || bs.length < 2 || list.length < 0.9 * as.length * bs.length) return null;
  if (list.some((c) => c.n < 30)) return null;
  const N = list.reduce((t, c) => t + c.n, 0);
  const grand = list.reduce((t, c) => t + c.s, 0) / N;
  if (!grand) return null;
  // Row and column effects fitted jointly (weighted backfitting), so a split
  // whose groups differ in make-up does not hide the interaction. Amounts
  // scale, so for positive values the fit is on the log scale.
  const positive = grand > 0 && list.every((c) => c.s > 0);
  const t = positive ? Math.log : (v) => v;
  const back = positive ? Math.exp : (v) => v;
  for (const c of list) c.y = t(c.s / c.n);
  const ra = new Map(as.map((v) => [v, 0]));
  const cb = new Map(bs.map((v) => [v, 0]));
  for (let it = 0; it < 25; it++) {
    for (const v of as) {
      const g = list.filter((c) => c.a === v);
      ra.set(v, g.reduce((s, c) => s + c.n * (c.y - cb.get(c.b)), 0) / g.reduce((s, c) => s + c.n, 0));
    }
    for (const v of bs) {
      const g = list.filter((c) => c.b === v);
      cb.set(v, g.reduce((s, c) => s + c.n * (c.y - ra.get(c.a)), 0) / g.reduce((s, c) => s + c.n, 0));
    }
  }
  let dev = 0;
  let noise = 0;
  let worst = null;
  for (const c of list) {
    const actual = c.s / c.n;
    const expected = back(ra.get(c.a) + cb.get(c.b));
    const variance = Math.max(0, c.ss / c.n - actual * actual);
    dev += c.n * (actual - expected) ** 2;
    noise += c.n * (variance / c.n);
    const gap = Math.abs(actual - expected) / Math.max(Math.abs(expected), 1e-9);
    if (!worst || gap > worst.gap) worst = { a: c.a, b: c.b, actual, expected, gap };
  }
  const rms = Math.sqrt(dev / N);
  const se = Math.sqrt(noise / N);
  const strength = rms / Math.abs(grand);
  if (strength < 0.1 || rms < 3 * se) return null;
  return { strength, cell: worst };
}

/**
 * The grain for a trend: coarsen from the table's own grain until each line
 * averages 30+ rows a point and there are at most ~30 points, so a trend of
 * events shows the trend rather than day-to-day noise. Never past a year.
 */
function seriesGrain(time, grain, rowCount, lines) {
  const DAYS = { hour: 1 / 24, day: 1, week: 7, month: 30.4, quarter: 91.3, year: 365 };
  const order = ['hour', 'day', 'week', 'month', 'quarter', 'year'];
  // Rows that are already period totals (a month per row) are not noisy
  // events; their grain is the one they were recorded at.
  if ((time.stats?.gapDays ?? 0) >= 6) return grain;
  const span = Math.max(1, time.stats?.spanDays || 1);
  let i = Math.max(0, order.indexOf(grain));
  while (i < order.length - 1) {
    const points = Math.max(1, span / DAYS[order[i]]);
    if (rowCount / (points * Math.max(1, lines)) >= 30 && points <= 30) break;
    if (span / DAYS[order[i + 1]] < 4) break;
    i++;
  }
  return order[i];
}
...
function layout(tiles) {
  ...
  for (const s of order) {
    ...
    const sized = ts.map((t, i) => {
      ...
      if (t.viz === 'heatmap' || MAP_VIZ.includes(t.viz)) return { ...t, w: 12, h: 5 };
