...
function breakdownInsightRaw({ data, x, ys, support, totals }, m, { rows, filters, dimLabel, ds, numericDim = false } = {}) {
  const y = ys[0];
...
export function distributionInsight({ values }, f, m) {
  ...
  // How much of the total the largest tenth carries: for amounts that add up,
  // the thing a reader of a spread most wants to know.
  const facts = { median: fmt(med), q1: fmt(q1), q3: fmt(q3), max: fmt(mx) };
  if (m?.additive && values.every((v) => v >= 0)) {
    const sorted = [...values].sort((a, b) => b - a);
    const total = sorted.reduce((a, b) => a + b, 0);
    const top = sorted.slice(0, Math.max(1, Math.round(sorted.length / 10))).reduce((a, b) => a + b, 0);
    const share = total > 0 ? top / total : 0;
    if (share >= 0.3) {
      const noun = m.unitNoun || 'rows';
      parts.push(`The largest 10% of ${noun} carry ${Math.round(share * 100)}% of the total.`);
      facts.topShare = `${Math.round(share * 100)}%`;
      score = Math.max(score, Math.min(0.85, (share - 0.1) * 1.4));
    }
  }
  return { text: parts.join(' '), score, facts, tone: 'neutral' };
}
...
export function seriesTrendInsight({ data, x, series }, m, { grain, seriesLabel } = {}) {
  ...
  const text =
    spread < 10
      ? `${m.label}: all ${read.length} ${plural(lower(seriesLabel))} moved together from ${first} to ${last} (${formatChange(up[up.length - 1].change)} to ${formatChange(up[0].change)}).`
      : `${m.label}: ${up[0].s} ${up[0].change < 0 ? 'fell least' : 'grew fastest'} (${formatChange(up[0].change)} from ${first} to ${last}); ${up[up.length - 1].s} ${up[up.length - 1].change < 0 ? 'fell most' : 'grew slowest'} (${formatChange(up[up.length - 1].change)}).`;
...
export function breakdownInsight(...args) {
  const r = breakdownInsightRaw(...args);
  return r && { ...r, text: cap(r.text) };
}
