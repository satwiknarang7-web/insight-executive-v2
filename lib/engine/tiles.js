...
export function chooseGrain(field) {
  ...
  const days = field.stats.spanDays || 0;
  if (days <= 3 && field.stats.hasTime) return 'hour';
...
export function grainsFor(field) {
  ...
  return GRAINS.filter((g) => {
    if (g === 'hour' && !field.stats.hasTime) return false;
...
export function allowedViz(tile, ds, measures, { levels = null } = {}) {
  ...
  switch (tile.kind) {
    ...
    case 'breakdown': {
      const n = levels ?? ds?.byName?.[tile.dim]?.stats?.distinct ?? 99;
      const ordered = ds?.byName?.[tile.dim]?.ordinal || ds?.byName?.[tile.dim]?.kind === 'number';
      if (tile.series) {
        const out = [];
      ...
      if (ms.length > 1) return ['groupedColumn', 'table'];
      out.push('table');
      return out;
    }
    default:
...
export function computeTile(rows, tile, ds, measures, filters = []) {
  ...
  if (tile.kind === 'kpi') {
    ...
    return { data: q.rows, value: q.rows[0]?.[m?.id] ?? null, support: q.totalSupport };
  }
  ...
  if (!tile.series) {
    ...
    const q = runQuery(rows, {
      ...
      filters: all,
      sort,
      limit: tile.kind === 'trend' ? 0 : tile.limit || MAX_BARS,
      other: tile.kind !== 'trend' && m?.additive,
  ...
  const keep = top.rows.slice(0, tile.seriesLimit || MAX_SERIES).map((r) => String(r[tile.series]));
