...
export const VIZ = {
  ...
  scatter: 'Scatter',
  table: 'Table',
  // Brought back from before the engine rebuild. Each is offered only where
  // the data can carry it, like every type above, and none is ever the first
  // choice for a tile, so a dashboard the planner builds is unchanged by them.
  pie: 'Pie',
  radial: 'Radial bars',
  gauge: 'Gauge',
  cards: 'Number cards',
...
/** The vizzes that draw a column of places on a map, and so read every place. */
export const MAP_VIZ = ['map', 'bubbleMap', 'shapeMap'];
 ...
 */
/**
 * Whether a measure can come out below zero, from its column's smallest value.
 *
 * A measure has no `negative` of its own — the column does (its minimum). The
 * checks below once read `m.negative`, which is never set, so a donut,
 * treemap or funnel was offered for a sum that can go negative. A count or a
 * rate cannot; a ratio can when either of its parts can.
 */
export function canBeNegative(m, ds) {
  if (!m) return false;
  const below = (name) => {
    const f = fieldOf(ds, name);
    const min = f?.stats?.min ?? f?.min;
    return typeof min === 'number' && min < 0;
  };
  if (m.type === 'agg') return below(m.field);
  if (m.type === 'ratio') return canBeNegative(m.num, ds) || canBeNegative(m.den, ds);
  return false;
}
...
export function allowedViz(tile, ds, measures, { levels = null } = {}) {
  ...
  const m = ms[0] ? { ...ms[0], negative: canBeNegative(ms[0], ds) } : ms[0];
  ...
  switch (tile.kind) {
    ...
    case 'distribution':
      return ['histogram'];
    case 'compare':
      // Several scores on one scale: a radar reads them as a profile.
      return ms.length >= 3 ? ['hbar', 'column', 'radar', 'table'] : ['hbar', 'column', 'table'];
    case 'relationship':
      // A third number as the size of each point.
      return tile.size ? ['scatter', 'bubble'] : ['scatter'];
    case 'table':
      return ['table'];
    case 'matrix':
    ...
    case 'trend': {
      if (!tile.series) {
        // Two measures on one time axis: columns for the first, a line for the second.
        if (ms.length === 2) return ['line', 'combo', 'table'];
        if (ms.length > 1) return ['line', 'table'];
        const out = ['line', 'column', 'table'];
        if (additive) out.splice(1, 0, 'area');
        // How the running total builds up, period by period.
        if (additive) out.splice(out.length - 1, 0, 'waterfall');
        return out;
      }
      // Which series leads in each period, and when the lead changes hands.
      const out = additive ? ['line', 'stackedArea', 'stackedColumn', 'ribbon', 'table'] : ['line', 'ribbon', 'table'];
      return out;
    }
    case 'breakdown': {
      ...
      if (ms.length === 2) return ['heatmap', 'groupedColumn', 'combo', 'table'];
      if (ms.length > 1) return ['heatmap', 'groupedColumn', 'table'];
      const positive = !m?.negative;
      // The same parts as the donut, without the hole.
      if (additive && n >= 2 && n <= 6 && positive) out.push('pie');
      // A few categories as arcs; lengths, so any measure that is not negative.
      if (n >= 2 && n <= 8 && positive) out.push('radial');
      // The leading part against the whole of them.
      if (additive && n >= 2 && positive) out.push('gauge');
      // A short list of values, as numbers rather than as marks.
      if (n >= 1 && n <= 12) out.push('cards');
      // The parts added one after another up to the total; a negative part steps down.
      if (additive && n >= 2 && n <= 12) out.push('waterfall');
      // A profile around the categories, which needs at least three spokes.
      if (n >= 3 && n <= 10 && positive) out.push('radar');
      if (f?.map && ms.length === 1) {
        // Circles sized by value: a size cannot be negative.
        if (positive) out.push('bubbleMap');
        // Only the places in the table, zoomed to fill the frame.
        out.push('shapeMap');
      }
      out.push('table');
      return out;
...
export function computeTile(rows, tile, ds, measures, filters = []) {
  ...
  if (tile.kind === 'relationship') {
    const data = fieldPairs(rows, tile.x, tile.y, all, { max: 1500, label: tile.label || null, color: tile.color || null, size: tile.size || null });
  ...
  if (!tile.series) {
    ...
    const q = runQuery(rows, {
      ...
      filters: all,
      sort,
      limit: tile.kind === 'trend' || MAP_VIZ.includes(tile.viz) ? 0 : tile.limit || MAX_BARS,
      other: tile.kind !== 'trend' && !MAP_VIZ.includes(tile.viz) && m?.additive,
