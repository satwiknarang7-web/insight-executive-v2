...
export function labelOf(name) {
  ...
  // Keep acronyms (USD, GDP, NPS) as written.
  return w
    .split(' ')
    .map((t, i) => (/^[A-Z0-9]{2,}$/.test(t) ? t : i === 0 ? t.charAt(0).toUpperCase() + t.slice(1).toLowerCase() : t.toLowerCase()))
    .join(' ');
}
...
export function roleField(f, { rows = 0 } = {}) {
  ...
  if (ID_NAME.test(w) && s.distinct > 50 && s.distinct >= 0.5 * s.filled) return { ...out, role: 'id', why: 'named like an id' };
  ...
  for (const m of measures) {
    for (const d of dims) {
      ...
      if (perLevel >= 3 && (named || (LONG_BY.test(words(d.name)) && (spread >= 20 || LONG_VALUE.test(m.name))))) {
        return { value: m.name, by: d.name, levels: [...groups.keys()] };
      }
    }
  }
  return null;
...
export function readDataset(rows, { name = 'dataset', overrides = {} } = {}) {
  ...
  // The time axis: the date column most rows have, then the widest span.
  const times = fields.filter((f) => f.role === 'time');
  const time =
    times.sort((a, b) => b.stats.fill - a.stats.fill || (b.stats.spanDays || b.stats.distinct || 0) - (a.stats.spanDays || a.stats.distinct || 0))[0] || null;
  ...
  const dims = fields.filter((f) => f.role === 'dimension' && !f.outcome).sort((a, b) => a.stats.distinct - b.stats.distinct);
  ...
  const named = outcomes.filter((f) => OUTCOME_NAME.test(words(f.name)));
