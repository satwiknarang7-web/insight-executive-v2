import { detectMap } from '../geo/match.js';
/**
...
export function roleField(f, { rows = 0 } = {}) {
  ...
  if (f.kind === 'number') {
    ...
    // An hour of the day (a part taken out of a timestamp) is when, not how
    // much: adding the hours of every order up means nothing.
    if (s.integer && /\bhour\b/.test(w) && s.min >= 0 && s.max <= 23) {
      return { ...out, role: 'dimension', ordinal: true, why: 'an hour of the day' };
    }
...
export function readDataset(rows, { name = 'dataset', overrides = {} } = {}) {
  ...
  // Columns of places: countries, or the states of one country. Only then
  // can a chart draw them on a map.
  for (const f of fields) {
    if (f.role !== 'dimension' || f.stats.distinct < 4 || f.stats.distinct > 400) continue;
    const counts = new Map();
    const step = rows.length > 20000 ? rows.length / 20000 : 1;
    for (let i = 0; i < rows.length; i += step) {
      const v = rows[Math.floor(i)]?.[f.name];
      if (v === null || v === undefined || v === '') continue;
      counts.set(String(v), (counts.get(String(v)) || 0) + 1);
    }
    const map = detectMap([...counts.entries()]);
    if (map) f.map = { code: map.code, name: map.name };
  }

  // A month or quarter column beside a date ("2024-03", "2024-Q1") says what
  // the date axis already says; the planner buckets the date itself, so a bar
  // per month here would repeat the trend. The column itself stays in the table.
  const timeField = fields.find((f) => f.role === 'time' && f.kind === 'date');
  if (timeField) {
    for (const f of fields) {
      if (f.role !== 'dimension' || f.alias) continue;
      let seen = 0;
      let period = true;
      for (let i = 0; i < rows.length && seen < 500; i += Math.max(1, Math.floor(rows.length / 500))) {
        const v = rows[i]?.[f.name];
        if (v === null || v === undefined || v === '') continue;
        seen += 1;
        if (!/^\d{4}-(0[1-9]|1[0-2]|Q[1-4])$/.test(String(v))) {
          period = false;
          break;
        }
      }
      if (period && seen) f.alias = timeField.name;
    }
  }
