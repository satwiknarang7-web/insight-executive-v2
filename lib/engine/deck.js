...
const EXPORT_TYPE = {
  ...
  stackedBar: 'hbar',
  donut: 'donut',
  // The exporters have no treemap or funnel; both are a ranking of parts,
  // which is what a bar chart shows.
  treemap: 'hbar',
  funnel: 'hbar',
  histogram: 'bar',
  scatter: 'scatter',
  heatmap: 'table',
  table: 'table',
  // The types brought back after the rebuild, drawn as the nearest chart the
  // exporters have: a pie is a donut, arcs and gauges and radars rank parts,
  // a ribbon is its lines, a combo keeps its columns, a bubble is its points,
  // and every map is its places ranked.
  pie: 'donut',
  radial: 'hbar',
  gauge: 'donut',
  cards: 'table',
...
export function dashboardToDeck(board, { measures = [], fields = [], fileName = null, rowCount = null } = {}) {
  ...
  const storyboard = tiles.map((t) => {
    ...
    return {
      ...
      chart: {
        ...
        // A map covers every region; a document's bar chart shows the top 15.
        resultData: MAP_VIZ.includes(t.viz) ? [...flat.rows].sort((a, b) => (Number(b[flat.y]) || 0) - (Number(a[flat.y]) || 0)).slice(0, 15) : flat.rows,
        xAxisKey: flat.x,
        yAxisKey: flat.y,
      },
      findings: {
  ...
  return {
    ...
    slideZero: {
      ...
      headline: board?.filterNote ? `Filtered to ${board.filterNote}` : board?.headline || bullets[0] || board?.summary || '',
      macroInsights: bullets,
    },
    filter: board?.filterNote ? { description: board.filterNote } : null,
    formatValue,
  };
}
