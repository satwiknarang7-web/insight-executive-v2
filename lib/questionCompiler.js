...
export function timeAxis(rows, col, { level = false } = {}) {
  ...
  for (const g of TIME_GRAINS) {
    ...
    if (cut >= MIN_POINTS || (level && cut === raw)) return { expr: `SUBSTRING(${br(col)}, 1, ${g.prefix})`, alias: g.alias, points: cut, prefix: g.prefix };
  }
...
function aggregateFor(rows, model, m, by = null, keep = () => true, { mean = false } = {}) {
  ...
  for (const r of rows) {
    ...
    const k = typeof by === 'function' ? by(r) : by ? String(r?.[by]) : '';
...
function longLevels(rows, model) {
  ...
  return { levels, shared: sizes.length >= 2 && extent(sizes).max / extent(sizes).min < 10 };
}

/**
 * The column a long table's rows are about: one row per level of it, per
 * period, per quantity — the country in (country, year, indicator). Fewest
 * levels first.
 */
function longEntity(rows, model) {
  const { by } = model.long;
  const time = model.grain.time;
  const candidates = Object.entries(model.columns)
    .filter(([c, info]) => c !== by && c !== time && ['category', 'flag'].includes(info.type) && info.distinct >= 2)
    .sort((a, b) => a[1].distinct - b[1].distinct);
  for (const [c] of candidates) {
    const seen = new Set();
    let unique = true;
    for (const r of rows) {
      const k = [r?.[c], time ? r?.[time] : '', r?.[by]].map(String).join('\u0001');
      if (seen.has(k)) {
        unique = false;
        break;
      }
...
/**
 * A quantity counted in each row's own currency — GDP in local currency
 * units — is in a different unit for each thing the rows are about. Added or
 * ranked across them it mixes naira with pounds.
 */
const localCurrency = (level) => LOCAL_CURRENCY_RE.test(String(level));

/** A long table's entity, when it has few enough levels to draw one line each. */
const LINES = 12;
...
function trend(q, rows, model) {
  ...
  // `single`: one row per point of each line, so the sum is the value itself
  // and is named as the value.
  const one = (extraWhere, titleSuffix, seriesCol = null, single = false) => {
    const scope = scopeFilter(rows, model, [m]);
    const keep = (r) => scope.keep(r);
    // Decided per period: one row can move a month's average without moving
    // the whole column's.
    const period = (r) => (axis.prefix ? String(r?.[q.over] ?? '').slice(0, axis.prefix) : String(r?.[q.over]));
    const agg = m ? aggregateFor(rows, model, m, period, keep) : null;
    const y = rate ? rate.name : m ? (single ? name(m) : labelFor(agg.fn, m)) : 'Records';
  ...
  // Each quantity on its own. Where the rows are about a handful of things
  // (six countries), one line each: a life expectancy or an inflation rate
  // added across countries is no one's, and a local-currency GDP added across
  // them mixes currencies. With more things than lines, a local-currency
  // quantity is left out rather than summed.
  const entity = longEntity(rows, model);
  const lines = entity && model.columns[entity].distinct <= LINES ? entity : null;
  return (q.level != null ? [q.level] : long.levels)
    .filter((l) => lines || !localCurrency(l))
    .slice(0, 3)
    .map((l) => one([`${br(by)} = ${lit(l)}`], ` — ${l}${lines ? ` by ${name(lines).toLowerCase()}` : ''}`, lines, !!lines));
}
...
function compare(q, rows, model) {
  ...
  // A local-currency quantity is compared within one thing or not at all.
  return (q.level != null ? [q.level] : long.levels)
    .filter((l) => !localCurrency(l))
    .slice(0, 3)
    .map((l) => one([`${br(by)} = ${lit(l)}`], (r) => String(r?.[by]) === String(l), ` — ${l}`))
    .filter(Boolean);
}
...
function relationship(q, rows, model) {
  ...
  return [{
    ...
    rowLevel: true,
    // Every row, not the first few hundred: a correlation over the head of a
    // file is an accident of row order, not a sample. The scatter component
    // caps what it draws; the figure under it is over the whole table.
    sql: `SELECT ${lead}${br(a)}, ${br(b)} FROM ${TABLE}${whereClause([`${br(a)} IS NOT NULL`, `${br(b)} IS NOT NULL`, ...scope.sql])}`,
    xAxisKey: a,
    yAxisKey: b,
  }];
}
...
export function headlineFigures(questions, rows, model) {
  ...
  for (const q of questions) {
    ...
    // A column most rows leave blank does not headline the table: its total
    // is a total of the few rows that have it.
    if (values.length < rows.length / 2) continue;
