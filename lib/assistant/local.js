...
import { parseTransformPhrase } from '../transformLanguage.js';
...
const PAGE_WORDS = [
  ...
  [/\b(data model|model page|joins?|relationships page|how tables join)\b/, 'model'],
  ...
  [/\b(home|upload|start|get data)\b/, 'home'],
];

// First match wins, so a phrase that contains another comes before it:
// "bubble map" before "bubble", "radial bar" before "bar", "combo" before "line".
const VIZ_WORDS = [
  [/\bbubble ?map\b/, 'bubbleMap'],
  [/\bshape ?map\b/, 'shapeMap'],
  [/\bheat ?map\b/, 'heatmap'],
  [/\b(choropleth|filled map|map)\b/, 'map'],
  [/\b(combo|combination|column and line|bars? and (a )?line|dual axis)\b/, 'combo'],
  [/\bribbon\b/, 'ribbon'],
  [/\bwaterfall\b/, 'waterfall'],
  [/\bradial\b/, 'radial'],
  [/\bgauge\b/, 'gauge'],
  [/\bradar|spider\b/, 'radar'],
  [/\bbubbles?\b/, 'bubble'],
  [/\b(number )?cards?\b/, 'cards'],
...
const TRANSFORM = /^(rename|drop|keep|delete|get rid of|convert|change|cast|turn|make (a |an )?(new )?column|create (a |an )?(new )?column|calculate|compute|derive|add (a )?(column|row number)|split|combine|merge|trim|uppercase|lowercase|proper case|replace|fill|remove (duplicates|blank|rows|the .* column)|dedupe|sort|band|bucket|extract|group by|read .* as|unpivot|pivot|filter rows|delete rows|delete the .* column)\b/;
...
/** "Please", "can you", "I want to": manners, not part of the command. */
const POLITE = /^(?:(?:please|pls|kindly|can you|could you|would you|will you|i want to|i'd like to|i would like to|i need to|help me|let'?s|go ahead and|now)\s+)+/i;

export function readCommand(text, { board, engine, dataset } = {}) {
  const raw = String(text || '').trim().replace(/[?.!]+$/, '').replace(POLITE, '').replace(/\s+please$/i, '');
  ...
  let m;
  if ((m = t.match(/\b(load|open|use|try|add|give me|start with|get)\b.*\b(demo|sample|example|test)\b/))) {
    const which = ['churn', 'subscription', 'messy', 'campaign', 'marketing', 'retail', 'sales'].find((w) => t.includes(w));
    const key = { subscription: 'churn', campaign: 'campaigns', marketing: 'campaigns', sales: 'retail' }[which] || which || 'retail';
    return { actions: [{ type: 'load_sample', sample: key }] };
  }
  ...
  // A data prep step the parser reads against these very columns is one,
  // whatever verb it starts with ("make a column profit = revenue - cost").
  const columns = dataset?.columns || [];
  const aboutBoard = /\b(chart|graph|kpi|card|tile|dashboard|slide)s?\b/.test(t) || /^(show|display|plot|visuali[sz]e|what|which|how|why|who|when|compare|top|trend|filter (to|by|on))\b/.test(t);
  if (columns.length && !aboutBoard && parseTransformPhrase(raw, { columns }).ok) return { actions: [{ type: 'transform', phrase: raw }] };
...
export function answerFromPassages(question, passages) {
  ...
  if (!top.length) return "I couldn't find anything on that. Try asking about a page (dashboard, report, data table), a column in your data, or say what to change, e.g. \"add revenue by region\".";
