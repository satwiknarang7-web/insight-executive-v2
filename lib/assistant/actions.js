...
/** What each page is called in the menu, so the assistant uses the same words. */
const PAGE_NAMES = {
  '/home': 'Get data', '/dashboard': 'the Dashboard', '/ask': 'Ask a question', '/explore': 'the Data table', '/quality': 'the Cleaning report',
  '/model': 'How tables join', '/summary': 'the Summary', '/report': 'the Report', '/present': 'the Slideshow', '/library': 'the Library', '/settings': 'Settings',
};
...
{"type":"edit_chart","id":"<chart id>","patch":{"viz":"line|area|column|hbar|stackedColumn|stackedBar|groupedColumn|stackedArea|donut|pie|treemap|funnel|waterfall|radial|gauge|cards|radar|ribbon|combo|map|bubbleMap|shapeMap|table|scatter|bubble|histogram|heatmap","dim":"<column>","series":"<column>|null","grain":"day|week|month|quarter|year","measures":["<measure id>"],"limit":10,"title":"..."}}
...
export function checkAction(a, { board, engine, filters = [], dataset } = {}) {
  ...
  switch (a.type) {
    case 'navigate': {
      ...
      return { ok: true, action: { type: 'navigate', path }, text: `Open ${PAGE_NAMES[path] || `the ${path.slice(1)} page`}` };
    }
    case 'load_sample': {
