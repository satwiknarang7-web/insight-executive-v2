 ...
 */
export function reviewClaims({ findings = [], storyboard = [], rowCount = 0 } = {}) {
  return checkValidity({ findings, storyboard, rowCount });
}
...
export function applySceptic({ storyboard = [], items = [] } = {}) {
  ...
  for (const item of items || []) {
    const fix = item.repair;
    if (!fix || fix.op !== 'clear_text') continue;
    const slide = board.find((s) => String(s?.id) === String(fix.id));
    if (!slide) continue;

    const patch = { insight_question: '' };
