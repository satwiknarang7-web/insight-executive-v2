 ...
 *   /api/purpose            a model call, per analysis run, before planning
 ...
 *   /api/extract            a vision call over a whole page, per document
...
export const LIMITS = {
  ...
  // Once per analysis run, on the critical path beside the semantics pass and
  // on the same briefing, so it is priced the same.
  purpose: { limit: 20, windowMs: FIFTEEN_MINUTES },
  // One per analysis run, like the purpose pass above it, and sized the same:
  // a handful of runs is normal use and a hundred is a stuck retry.
  compose: { limit: 20, windowMs: FIFTEEN_MINUTES },
  ...
  // Vision reads a page at a time and costs per page, so this is the tightest
  // of them all. Twelve documents in a quarter of an hour is more than anyone
  // photographs by hand; a stuck retry does it in a second.
  extract: { limit: 12, windowMs: FIFTEEN_MINUTES },
