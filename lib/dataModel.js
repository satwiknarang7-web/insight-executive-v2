...
export function joinKey(value) {
  ...
  if (s === '') return null;
  // "1001", "1001.0", "0001" and 1001 must all collapse to the same key. Done
  // by reading the digits rather than through Number, because an integer longer
  // than fifteen digits loses its tail to a double: two distinct ids became one
  // key, and the join then matched rows that have nothing to do with each
  // other. The cleaner keeps ids that long as text for the same reason.
  const digits = /^(-?)0*(\d+?)(?:\.0+)?$/.exec(s);
  if (digits) return digits[2] === '0' ? '0' : `${digits[1]}${digits[2]}`;
...
/** The id a relationship between these two columns has, inferred or not. */
export function relationshipId(from, to) {
  return `${from.table}.${from.column}->${to.table}.${to.column}`;
}

/**
 * Measure a relationship the user is proposing, before it is accepted.
 *
 * Inference declines to offer a pairing for good reasons — the values do not
 * overlap, the parent side is not unique — and a person overruling that is
 * entitled to do so. What they are not entitled to is doing it blind: a join on
 * a parent column with duplicate values silently multiplies the fact rows, and
 * the resulting totals are wrong in exactly the way this product exists to
 * prevent. So the pairing is measured against the real rows and the answer is
 * shown before anything is applied.
 *
 * `problems` refuses; `warnings` are yours to overrule.
 *
 * @returns {{ ok: boolean, cardinality: string|null, overlap: number,
 *   matched: number, distinct: number, problems: string[], warnings: string[] }}
 */
export function testRelationship(tables, { from, to } = {}) {
  const fail = (message) => ({
    ok: false,
    cardinality: null,
    overlap: 0,
  ...
  if (!from?.table || !from?.column || !to?.table || !to?.column) {
    return fail('Choose a column on both sides.');
  }
  if (from.table === to.table) {
    return fail('A table cannot be joined to itself.');
  }

  const child = tables.find((t) => t.name === from.table);
  const parent = tables.find((t) => t.name === to.table);
  if (!child || !parent) return fail('That table is not loaded.');

  const cs = profileKeys(child.rows || [], [from.column])[from.column];
  const ps = profileKeys(parent.rows || [], [to.column])[to.column];
  if (!cs || !ps) return fail('That column is not in the table.');
  if (!cs.values || !ps.values) {
    return fail('A decimal column cannot be a key — its values are measurements, not identities.');
  }

  const problems = [];
  const warnings = [];

  // The one hard rule. Without a unique parent every fact row matches several
  // parent rows, the join multiplies them, and every total downstream is
  // inflated — the fan trap, arrived at deliberately.
  if (!ps.unique) {
    problems.push(
      `${to.table}.${to.column} has repeated values, so one row on the left would match several on ` +
        'the right and every total would be multiplied. The right-hand column has to identify a row.'
    );
  }

  const overlap = containment(cs.values, ps.values);
  if (overlap === 0) {
    problems.push('None of the values on the left appear on the right, so nothing would join.');
  } else if (overlap < POOR_MATCH) {
    warnings.push(
      `Only ${Math.round(overlap * 100)}% of the values on the left are found on the right. ` +
        'The rest would come through empty.'
    );
  }
  if (cs.nullRatio > 0.5) {
    warnings.push(`${from.table}.${from.column} is more than half empty.`);
  }
  if (cs.sampled || ps.sampled) {
    warnings.push('Measured on a sample of the rows, so the real match rate may differ a little.');
  }

  let matched = 0;
  for (const v of cs.values) if (ps.values.has(v)) matched++;

  return {
    ok: problems.length === 0,
    cardinality: problems.length === 0 ? (cs.unique ? 'one-to-one' : 'many-to-one') : null,
    overlap: Number(overlap.toFixed(3)),
    matched,
    distinct: cs.values.size,
    problems,
    warnings,
  };
}
...
/**
 * Turn a measured pairing into a relationship the model can carry.
 *
 * It is shaped exactly like an inferred one so nothing downstream has to know
 * the difference, with `source: 'manual'` so the review page can say who is
 * responsible for it. Confidence is 1: this is not a guess, it is an
 * instruction.
 */
export function manualRelationship({ from, to }, measured) {
  return {
    id: relationshipId(from, to),
    from: { ...from },
    to: { ...to },
    cardinality: measured.cardinality || 'many-to-one',
    confidence: 1,
    overlap: measured.overlap,
    nameAffinity: nameAffinity(from.column, to.table, to.column),
    source: 'manual',
    reasons: [
      'Added by you',
      `${Math.round(measured.overlap * 100)}% of values found in the parent column`,
    ],
  };
}
 ...
 */
export function buildDataModel(tables, { factTable = null, relationships: supplied = null } = {}) {
  const statsByTable = {};
  for (const t of tables) statsByTable[t.name] = profileKeys(t.rows || [], t.columns || []);

  const inferred = inferRelationships(tables, { statsByTable });
  const { alternatives, warnings } = inferred;

  // A supplied list is the user's model: relationships they switched off are
  // absent from it, and ones they added by hand are in it. Everything
  // downstream — which table is the fact, which tables count as related, what
  // the view joins — has to follow that list rather than the guess, or a
  // hand-added relationship would join the rows and still leave its table
  // labelled unrelated.
  const relationships = supplied || inferred.relationships;
...
export function buildAnalysisView(tablesByName, model, { maxDepth = DEFAULT_MAX_DEPTH } = {}) {
  ...
  /**
   * With nothing to join, the view IS the fact table.
   *
   * This cloned every fact row unconditionally, so a single-file dataset was
   * held twice in the worker — once as the table and once as an identical
   * "view" — for no benefit at all. On a 250,000-row export that is a quarter
   * of a million object copies to produce the array that was already there.
   *
   * The clone below exists because the join steps widen rows in place, which
   * must not reach back into the source table. With no steps, nothing widens
   * anything, so there is nothing to protect against.
   */
  if (plan.length === 0) {
    return { rows: factRows, columns, provenance, joins, skipped };
  }
...
/** Below this, a join matched so few rows that it is probably not a join. */
export const POOR_MATCH = 0.9;
/** Below this, the inference was guessing more than it was reading. */
export const LOW_CONFIDENCE = 0.75;

/**
 * Everything about a model that a person should look at before trusting an
 * analysis built on it.
 *
 * The order matters: a measured match rate comes first, because it is what
 * actually happened, while confidence is only what the inference believed
 * beforehand. A relationship the engine was 95% sure of that matched 4% of rows
 * is wrong, and the measured number is the one that says so.
 *
 * Returns plain sentences rather than codes, because they are shown to the user
 * verbatim in two places and inventing a second vocabulary for them would only
 * let the two drift apart.
 */
export function modelConcerns({ model, tables = [], joins = [] } = {}) {
  if (!model) return [];
  const concerns = [];

  const rateFor = (rel) =>
    joins.find((j) => j.from?.table === rel.from.table && j.to?.table === rel.to.table)?.matchRate ??
    null;

  for (const rel of model.relationships || []) {
    const rate = rateFor(rel);

    if (rate !== null) {
      // A measured result supersedes the prior entirely. A join that matched
      // every row is sound however tentatively it was guessed, so raising its
      // confidence as a concern would be noise — and noise here trains people
      // to skip the one warning that matters.
      if (rate < POOR_MATCH) {
        concerns.push(
          `${rel.from.table} → ${rel.to.table} matched only ${Math.round(rate * 100)}% of rows.`
        );
      }
    } else if (rel.confidence < LOW_CONFIDENCE) {
      concerns.push(`${rel.from.table} → ${rel.to.table} was inferred with low confidence.`);
    }
  }

  for (const warning of model.warnings || []) concerns.push(warning.message);

  for (const table of tables) {
    if (table.role === 'unrelated') {
      concerns.push(`${table.name} is not related to anything, so none of its columns are analysed.`);
    }
  }
