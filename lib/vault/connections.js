...
import { storedSource } from '../connectors/registry.js';
import { sealCredential, openCredential, stripSecrets, extractSecrets, mergeSecrets } from './crypto.js';
...
/**
 * The database will not accept this source.
 *
 * `public.connections.source` is constrained to the set of connectors the app
 * knows how to dial. A connector the database has not been told about therefore
 * cannot be saved at all, and every attempt failed with a check violation —
 * which the generic handler reported as "That request could not be completed",
 * sending whoever saw it to check their host, their password and their network
 * for a problem that was none of those.
 *
 * The usual answer is `storeAs` in the registry: a flavour of an existing
 * source stores under that source and needs no schema change. This is the guard
 * for a connector that declares neither.
 *
 * It is not a credential problem, so unlike the other failures here it says
 * exactly what is wrong. Nothing about it is useful to someone probing.
 */
export class SourceNotPermitted extends Error {
  constructor(source) {
    super(
      `This database does not accept “${source}” as a connection source. ` +
        'Either give that connector a `storeAs` in the registry pointing at a source ' +
        'the database already knows, or widen the check constraint on public.connections.'
    );
...
/** Is this Postgres refusing the `source` value rather than anything else? */
export function isSourceCheckViolation(error) {
  const text = `${error?.code || ''} ${error?.message || ''}`.toLowerCase();
  return text.includes('connections_source_check') || (text.includes('23514') && text.includes('source'));
}
...
/**
 * Amend the non-secret half of a saved connection's config.
 *
 * `saveConnection` cannot do this. It rewrites both halves and refuses a config
 * with no credentials in it — correctly, since that is how a connection with an
 * empty vault entry would be created. But the client secret is never readable,
 * so a form that only wants to record *which warehouse was chosen* has no way
 * to supply one.
 *
 * Hence a narrow second door: it merges a patch into `config` and touches
 * nothing else. Secret-shaped keys are stripped on the way in, so this can
 * never be used to smuggle a credential into a table the whole organisation can
 * read — the vault stays the only place one is written.
 */
export async function updateConnectionConfig(connectionId, { orgId, patch }) {
  const who = await membership(orgId);
  if (!who) throw new Forbidden();
  ...
  const merged = stripSecrets({ ...(existing.config || {}), ...(patch || {}) });
  const { data, error } = await supabase
    .from('connections')
...
export async function saveConnection({ orgId, name, source, config, connectionId = null }) {
  ...
  // A flavour connector (Neon) stores under the source its driver is written
  // for, and records what it actually is so the UI can still say so.
  const stored = storedSource(source);
  const flavoured = stored === source ? config : { ...config, provider: source };

  const safeConfig = stripSecrets(flavoured);
  const secrets = extractSecrets(flavoured);
  ...
  const { data, error } = await svc.rpc('svc_upsert_connection_secret', {
    p_org_id: orgId,
    p_name: name,
    p_source: stored,
    p_config: safeConfig,
  ...
  if (error) {
    // A source the database has not been told about is a missing migration, not
    // a bad credential, and saying so saves an hour of checking the host and
    // the password for a problem that is neither.
    if (isSourceCheckViolation(error)) throw new SourceNotPermitted(source);
...
export async function openConnection(connectionId, { orgId, reason = 'used' }) {
  ...
  const { data: secretRows, error: secretError } = await svc
    .rpc('svc_read_connection_secret', { p_connection_id: connectionId });
  ...
  return { ...mergeSecrets(row.config, credentials), __source: row.source };
}
...
export async function revokeConnection(connectionId, { orgId }) {
  ...
  await svc.rpc('svc_delete_connection_secret', { p_connection_id: connectionId });
