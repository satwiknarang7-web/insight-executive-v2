 ...
 *
 * `buildMeasures` (lib/engine/measures.js) already writes the measures a report uses — order value,
 ...
 */
/** A column name as a person would write it: `order_date` → "Order Date". */
...
import { MAP_CODES, detectMap, regionIds } from './geo/match.js';
import GEO from './geo/index.json' with { type: 'json' };

/** US and Canadian postal abbreviations (TX, ON), which the two maps use as ids. */
const POSTAL = new Set(['US', 'CA'].flatMap((code) => (GEO[code]?.regions || []).map(([id]) => String(id).toUpperCase())));
...
export function deriveSteps({ columns = [], profile = null, vocabulary = null } = {}) {
  ...
  for (const step of splitSteps(profile, vocabulary, existing)) take(step);
 ...
 */
function splitSteps(profile, vocabulary, existing = new Set()) {
  const out = [];
  ...
  for (const column of profile?.dimensions || []) {
    ...
    const counted = (levels[column] || []).map((v) => [String(v.value), v.count || 1]);
    const values = counted.map(([v]) => v);
    ...
    for (const sep of SEPARATORS) {
      ...
      out.push({
        ...
        column,
        separator: sep.text,
        into: partNames(column, counted, sep.text, width, existing),
        dropOriginal: false,
...
/** What one level of each map is called. Anything not listed is a Region. */
const LEVEL = {
  WORLD: 'Country',
  US: 'State', IN: 'State', AU: 'State', MX: 'State', BR: 'State', DE: 'State', NG: 'State', AT: 'State',
  CA: 'Province', CN: 'Province', ES: 'Province', IT: 'Province', NL: 'Province', KR: 'Province', TH: 'Province', ID: 'Province', LK: 'Province', SA: 'Province',
  JP: 'Prefecture',
  GBC: 'County', SE: 'County',
};

/** Words that say a column holds places, so a part beside a country is a city. */
const PLACE_NAME = /\b(location|city|town|address|place|office|store|branch|site|hq|headquarters|origin|destination)\b/;

/** Words a column name can spell its own parts with: `city_state`, `first_last`. */
const PART_WORD = new Set(['city', 'town', 'state', 'province', 'region', 'country', 'county', 'district', 'zip', 'postcode', 'first', 'last', 'given', 'family', 'surname', 'street']);

/**
 * What to call each part of a split column.
 *
 * "Location 2" tells a reader nothing; "State" tells them what to group by.
 * Three readings, most certain first:
 *  - the column's own name, when it spells one part word per part
 *    (`city_state` → City, State);
 *  - the values: a part whose values are nearly all regions of one map is a
 *    State, a Province, a Country…, and a plain part in front of one of those
 *    is the City;
 *  - otherwise the column's name and a number, as before.
 * A name that is already a column, or taken by another part, gets the
 * column's name in front of it ("Location State").
 */
export function partNames(column, counted, separator, width, existing = new Set()) {
  const label = prettyColumn(column);
  const fallback = Array.from({ length: width }, (_, i) => `${label} ${i + 1}`);
  let names = [...fallback];

  const words = spaced(column).toLowerCase().split(/\s+/).filter((w) => w && w !== 'and');
  if (words.length === width && words.every((w) => PART_WORD.has(w))) {
    names = words.map((w) => prettyColumn(w));
  } else {
    const levels = Array.from({ length: width }, (_, i) => {
      const top = new Map();
      for (const [value, n] of counted) {
        const piece = value.split(separator)[i]?.trim();
        if (piece) top.set(piece, (top.get(piece) || 0) + n);
      }
      const entries = [...top.entries()];
      const map = detectMap(entries, { minRegions: 3 });
      if (map) return LEVEL[map.code] || 'Region';
      // One or two countries ("USA", "Canada") are too few regions to call a
      // map, and are still plainly countries.
      const world = detectMap(entries, { minRegions: 1 });
      if (world?.code === 'WORLD') return 'Country';
      // States of one country and provinces of another (TX, ON, BC) fill no
      // single map, and are still one level of place.
      const total = entries.reduce((sum, [, n]) => sum + n, 0);
      const inSome = entries.filter(([v]) => POSTAL.has(v.toUpperCase()) || MAP_CODES.some((code) => code !== 'WORLD' && code !== 'GBC' && regionIds(code, v).length));
      const share = inSome.reduce((sum, [, n]) => sum + n, 0) / (total || 1);
      return inSome.length >= 3 && share >= 0.85 ? 'State' : null;
    });
    const anyPlace = levels.some(Boolean);
    const placeColumn = PLACE_NAME.test(spaced(column).toLowerCase());
    const subnational = levels.some((l) => l && l !== 'Country');
    names = levels.map((level, i) => {
      if (level) return level;
      // A plain part beside a state or a province is the town it is in; beside
      // only a country, it is a city when the column says it holds places.
      if (anyPlace && (subnational || placeColumn) && levels.filter((l) => !l).length === 1) return 'City';
      return fallback[i];
    });
  }
  ...
  return names.map((name, i) => {
    let out = name;
    if (taken.has(out.toLowerCase()) || names.indexOf(name) !== i) out = `${label} ${name}`;
    if (taken.has(out.toLowerCase())) out = fallback[i];
