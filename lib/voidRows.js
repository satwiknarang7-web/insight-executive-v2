 ...
 */
export function excludeVoidRows(rows = [], profile = null, claimed = null) {
  ...
  return {
    rows: kept,
    excluded: { ...found, kept: kept.length, total: rows.length },
  };
}
...
export function describeExclusion(excluded) {
  if (!excluded) return '';
  const { rows, sharePct, levels, column, kept } = excluded;
  return (
 ...
 * is not dismissible.
 */
export function exclusionNotice(excluded) {
  if (!excluded) return [];
  return [
    {
      kind: 'rows-excluded',
