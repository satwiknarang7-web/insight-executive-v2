 ...
 *
 * **A caller may bring their own Gemini key**, in which case that key is used on
 * its own and the deployment's providers are not tried at all. Both halves of
 * that matter. Falling back would spend the operator's money on a request the
 * viewer explicitly asked to put on their own account — which is the entire
 * point of supplying a key — and it would hide a broken one behind a result
 * that looked fine, leaving them to discover months later that nothing they
 * typed was ever used. A key that does not work is reported as not working.
 *
 ...
 *
 * Shape-checked before use so that a header full of junk is treated as no key
 * rather than dialled out to Google. Returns '' for anything that is not one,
 * and never returns what it rejected — the value must not reach a log line, and
 * the surest way to guarantee that is for it never to be in one.
 */
 ...
 *
 * Either the viewer brought a key or the deployment has one. A deployment with
 * no keys of its own is a supported configuration, not a broken one: every
 * viewer supplies their own, and anyone who does not gets the deterministic
 * text, which is complete on its own.
 */
export function canGenerate(request) {
  return !!callerGeminiKey(request) || hasAnyProvider();
}
...
async function viaGemini(prompt, system, apiKey = '') {
  // A caller's key gets its own client every time rather than the memoised one:
  // the module-level client belongs to the deployment's key, and caching per
  // viewer would mean holding other people's credentials in process memory for
...
export async function generateJson(prompt, system, { geminiKey = '' } = {}) {
  // Their key, their account, their call — and no quiet fallback to ours.
  if (geminiKey) return (await viaGemini(prompt, system, geminiKey)) || null;
