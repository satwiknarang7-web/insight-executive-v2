  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let c = 0; c < columnNames.length; c++) {
      ...
      if (typeof val === 'string') {
        ...
        /**
         * The minus signs that are not the hyphen-minus.
         *
         * Excel writes U+2212 MINUS SIGN when a cell is formatted as a number,
         * and European systems and typeset exports write the en dash. Neither
         * matches `SIGNED_CURRENCY`, so `−567.89` stayed text — and a column
         * holding a few of them alongside ordinary numbers drops below the
         * numeric-purity bar and becomes a category.
         *
         * Only a LEADING one is rewritten. A dash between two numbers is a
         * range ("10–20"), not a negative, and turning that into a subtraction
...
export function unifyCategories(rows, columnNames, metrics) {
  ...
  for (const col of columnNames) {
    ...
    const samples = [];
    let merged = 0;
    // A spelling in ordinary case (North) reads better than SHOUTING or all
    // lower case, and one column should not show EAST beside west beside North.
    const mixed = (t) => /[A-Z]/.test(t) && /[a-z]/.test(t);
    const chosen = new Map(); // group key -> chosen raw
    for (const [k, variants] of groups) {
      ...
      const total = [...variants.values()].reduce((a2, b2) => a2 + b2, 0);
      const cased = [...variants].filter(([raw, count]) => mixed(raw) && raw.trim() === raw && count >= 0.2 * total).sort((x, y) => y[1] - x[1])[0];
      if (cased && !mixed(best.raw)) best = { raw: cased[0], count: cased[1] };
      chosen.set(k, best.raw.trim().replace(/\s+/g, ' '));
    }
    const mostlyMixed = [...chosen.values()].filter(mixed).length >= chosen.size / 2;
    const title = (t) => t.replace(/[^\s\-/]+/g, (w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase());
    if (mostlyMixed) {
      for (const [k, raw] of chosen) {
        // Short all-capitals are acronyms (UK, SKU, OK); leave them.
        if (!mixed(raw) && /[a-z]/i.test(raw) && raw.replace(/[^a-z]/gi, '').length > 3) chosen.set(k, title(raw));
      }
    }
    for (const [k, variants] of groups) {
      const best = { raw: chosen.get(k) };
      for (const [raw, count] of variants) {
...
export function unifyBooleans(rows, columnNames, metrics) {
  for (const col of columnNames) {
    ...
    /**
     * A word beats a digit, then the commonest wins.
     *
     * Frequency alone folded a churn column to `Yes` and `0` — two spellings of
     * opposite answers, one a word and one a digit, because `0` happened to be
     * the commonest way that file wrote false. It reads badly, and a level
     * shaped like a number is then indistinguishable from a number the cleaner
     * could not parse.
     *
     * The file's own vocabulary is still kept: a table written TRUE/FALSE folds
     * to TRUE/FALSE, and one written Y/N folds to Y/N. Nothing is invented —
     * only a spelling the column already contains can win.
     */
    const winner = (bucket) => {
      let best = null;
      for (const [raw, count] of bucket) {
        const word = !/^\d+$/.test(raw.trim());
        const bestWord = best ? !/^\d+$/.test(best.raw.trim()) : false;
        if (
          !best ||
          (word && !bestWord) ||
          (word === bestWord &&
            (count > best.count ||
              (count === best.count && raw.trim() === raw && best.raw.trim() !== best.raw)))
        ) {
          best = { raw, count };
        }
      }
    ...
    const samples = [];
    let merged = 0;
    // The two sides written the same way: Yes/No, not Yes/no.
    const keeps = { true: winner(sides.true), false: winner(sides.false) };
    const cap = (t) => typeof t === 'string' && /^[A-Z][a-z]/.test(t);
    const low = (t) => typeof t === 'string' && /^[a-z]+$/.test(t);
    if (cap(keeps.true) && low(keeps.false)) keeps.false = keeps.false.charAt(0).toUpperCase() + keeps.false.slice(1);
    if (cap(keeps.false) && low(keeps.true)) keeps.true = keeps.true.charAt(0).toUpperCase() + keeps.true.slice(1);
    for (const key of ['true', 'false']) {
      const keep = keeps[key];
