 ...
 */
export function planCharts(rows, { max = 7, provenance = {}, roles = {}, derived = null, claims = null } = {}) {
  ...
  const repeatedAt = detectRepeatedMeasures(rows, p);
  ...
  const usable = dims.filter(
    (d) => d !== timeCol && (cardinality[d] || 0) >= 2 && (cardinality[d] || 0) <= 25
  );
  ...
  autoMeasures.slice(0, 3).forEach((m, i) => {
    rankDims.slice(0, 3).forEach((cat, j) => {
    if (!cat) return;
  ...
  while (picked.length < max && remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const eff =
        ...
        (dimCount[dimOf(c)] || 0) ** 1.6 * DIM_PENALTY -
        redundancy;
      if (eff > bestEff) {
        bestEff = eff;
        bestIdx = i;
     ...
     */
    const dimUses = dimCount[dimOf(remaining[bestIdx])] || 0;
 ...
 */
export function planKpis(rows, { provenance = {}, roles = {}, claims = null } = {}) {
  if (!rows || rows.length === 0) return [];
  const p = profileColumns(rows);
  ...
  const semantics = classifyColumns({
    ...
    repeatedAt: detectRepeatedMeasures(rows, p),
    denominatedBy: claims,
  });
  const notSummable = new Set([
    ...semantics.preAggregate,
