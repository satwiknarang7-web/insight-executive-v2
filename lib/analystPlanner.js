...
export function planCharts(rows, { max = 7, provenance = {}, roles = {}, derived = null, claims = null, profile = null, repeated = null, purpose = null } = {}) {
  ...
  /**
   * Measures, ordered by what a reader came for.
   *
   * `spread` is max minus min in the column's own units, so ranking on it picks
   * whichever measure happens to be counted in the largest numbers. That is how
   * a context window in tokens — a million on most rows and dull on all of them
   * — became the headline of a report about what subscriptions cost. The
   * purpose pass decides first; spread breaks the ties it has no opinion about.
   */
  const bySpread = (arr) => [...arr].sort((a, b) => (stats[b]?.spread || 0) - (stats[a]?.spread || 0));
  const byInterest = (arr) =>
    orderByPurpose(arr, intent, 'measure', (a, b) => (stats[b]?.spread || 0) - (stats[a]?.spread || 0));
...
export function planKpis(rows, { provenance = {}, roles = {}, claims = null, profile = null, repeated = null, purpose = null } = {}) {
  ...
  /**
   * The most *varied* measure, not the one with the widest absolute range.
   *
   * `spread` is max minus min, which is a number in the column's own units, so
   * ranking on it picks whichever column happens to be measured in the largest
   * ones. A comparison of AI plans led its strip with "Average Context Window
   * 753.2K" — a column that reads 1M for most of its rows, with a couple of
   * smaller ones dragging the mean down. Its range was the widest in the file
   * by six orders of magnitude and its average told the reader nothing.
   *
   * Dividing by the mean asks the question that actually matters: how much does
   * this measure move, relative to its own size? A price column that runs from
   * zero to a few hundred beats a context window that is the same number nearly
   * everywhere, which is the card a reader wants.
   */
  const variation = (m) => {
    const stat = stats[m];
    if (!stat || !Number.isFinite(stat.spread)) return 0;
    const scale = Math.abs(stat.mean);
    // A column centred on zero has no meaningful relative spread; fall back to
    // the absolute one rather than dividing by nothing.
    return scale > 1e-9 ? stat.spread / scale : stat.spread;
  };
  const avgM = orderByPurpose(
    avgPool.length ? avgPool : comparable,
    intent,
    'measure',
    (a, b) => variation(b) - variation(a)
  )[0];
  if (avgM) {
