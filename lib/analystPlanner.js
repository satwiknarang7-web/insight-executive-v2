 ...
 */
export function planCharts(rows, { max = 7, provenance = {}, roles = {}, derived = null, claims = null, profile = null, repeated = null, brief = null } = {}) {
 ...
 */
export function planKpis(rows, { provenance = {}, roles = {}, claims = null, profile = null, repeated = null, brief = null } = {}) {
  if (!rows || rows.length === 0) return [];
  // Profiling 250,000 rows costs about two and a half seconds, and this used to
  // happen four separate times in one analysis — here, in the other two entry
  // points, and once in the pipeline — for ten seconds of the twenty-two a
   ...
   */
  const byMagnitude = [...additive].sort(
  ...
  const avgM = [...(avgPool.length ? avgPool : comparable)].sort(
    (a, b) => (stats[b]?.spread || 0) - (stats[a]?.spread || 0)
  )[0];
  if (avgM) {
