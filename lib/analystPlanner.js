 ...
 */
export function planCharts(rows, { max = 7, provenance = {}, roles = {}, derived = null, claims = null, profile = null, repeated = null, purpose = null, brief = null } = {}) {
   ...
   */
  /**
   * Which measure a band column was cut from, if it was.
   *
   * The planner's own preparation step buckets a continuous measure into bands
   * so it can be grouped by, and names the result after the column it cut:
   * `revenue` becomes `Revenue Band`. Nothing then stopped the planner
   * grouping `revenue` BY `Revenue Band`, and on five of ten evaluation
   * datasets that was the first chart in the deck:
   *
   *   "Total Revenue by Revenue Band"  — STRONG EVIDENCE —
   *   "20000–50000 leads revenue bands on total revenue at 9.7M, 43.4% of the total"
   *
   * Every one of those is true by construction. The sum of a measure inside its
   * own top band is the largest sum there can be; the mean of the 500+ bucket
   * is above the overall mean by definition. It is arithmetic reported as a
   * finding, and the evidence tier cannot tell, because the arithmetic is
   * impeccable.
   *
   * A histogram is not this: counting rows per band is a real question about
   * shape, and `Distribution of X` keeps its slide. What goes is summing or
   * averaging X over bands of X.
   *
   * Matched on the suffix this planner writes itself, so it is reading its own
   * output rather than guessing at a reader's column names. A file that
   * genuinely holds both `revenue` and `Revenue Band` gets the same treatment,
   * and should.
   */
  const bandSource = (dimension) => {
    const stripped = String(dimension).replace(/\s+Band$/i, '');
    if (stripped === String(dimension)) return null;
    const key = (n) => String(n).toLowerCase().replace(/[\s_-]+/g, '');
    return measures.find((m) => key(m) === key(stripped)) || null;
  };
  /** Is charting `measure` broken down by `dimension` a tautology? */
  const bandsItself = (dimension, measure) => !!measure && bandSource(dimension) === measure;
  ...
  /**
   * The dimensions the magnitude charts may use.
   *
   * Everything downstream of here aggregates `sumCol`, so a band cut from
   * `sumCol` is excluded — and only from these. `Average Discount Pct by
   * Revenue Band` is a real question and keeps its pool below; `Total Revenue
   * by Revenue Band` is not one.
   */
  const summable = usable.filter((d) => !bandsItself(d, sumCol));
  const rankDims = [...summable].sort(
    (a, b) => dimScore(b) - dimScore(a) || (cardinality[b] || 0) - (cardinality[a] || 0)
  );
  const smallDims = byCard(summable.filter((d) => cardinality[d] <= 8));
  /**
  ...
  // The treemap aggregates `sumCol` too, so its pool gets the same exclusion.
  const midCard = dims.filter(
    (d) => d !== timeCol && cardinality[d] > 8 && cardinality[d] <= 25 && !bandsItself(d, sumCol)
  );
  ...
  if (timeCol) {
    ...
    const grain = isISODate ? timeGrain(rows, timeCol) : null;
    const prefix = grain ? grain.prefix : 0;
    let xAlias = timeCol;
  ...
  if (timeCol && mixDims.length) {
    const tSample = String(rows[0]?.[timeCol] ?? '');
    const isISODate = /^\d{4}-\d{2}-\d{2}/.test(tSample);
    const mixGrain = isISODate ? timeGrain(rows, timeCol) : null;
    const prefix = mixGrain ? mixGrain.prefix : 0;
    const xAlias = mixGrain ? mixGrain.alias : timeCol;
  ...
  if (primary) {
    ...
    rankDims.slice(0, countOnly ? 4 : 2).forEach((cat, i) => {
      ...
      // "Average Points by Points Band": the mean of the 500+ bucket is above
      // the overall mean by definition, and the card called it moderate
      // evidence. Checked per pair, because this loop chooses its own measure.
      if (bandsItself(cat, measure)) return;
  ...
  if (outcome) {
    /**
     * What measures the outcome, what it is called, and how it is formatted —
     * from one function, because on a continuous outcome those three answers
     * are no longer "a rate" and they have to change together. A mean labelled
     * "Rate" and drawn on a percentage axis is three bugs that raise no error:
     * the bars draw and every number on them is a hundred times too small.
     */
    const aggregate = outcomeAggregate(outcome);
    const rateName = aggregate.name;
    const rateExpr = aggregate.expr;
     ...
     */
    // `determines` rather than a bare threshold on Cramer's V: on a sparse
    // contingency table — a continuous column against a handful of levels — V
    // reaches 1.0 between columns that have nothing to do with each other, and
    // every dimension in the deck would be discarded as a restatement.
    const restatesOutcome = (d) => determines(signalRows, d, outcome.column);
  ...
  if (timeCol && sumCol && cardinality[timeCol] >= 3) {
    ...
    const fallGrain = iso ? timeGrain(rows, timeCol) : null;
    const periodExpr = fallGrain ? `SUBSTRING(${br(timeCol)}, 1, ${fallGrain.prefix})` : br(timeCol);
    const alias = fallGrain ? fallGrain.alias : pretty(timeCol);
    add({
       ...
       */
      signal: { kind: 'contribution', column: timeCol, prefix: fallGrain ? fallGrain.prefix : 0 },
      score: 64,
    });
  }
...
function readSignal(signal, rows, sumCol) {
  ...
  if (signal.kind === 'outcomeBand') {
    ...
    // The kind travels on the signal so the scorer measures the outcome the
    // same way the SQL does. Without it a continuous outcome is scored as a
    // share of rows equal to `undefined` — zero everywhere, no spread, and the
    // chart is dropped before anyone sees it.
    const groups = outcomeGroups(banded, '__band', signal.column, signal.event, { kind: signal.outcomeKind });
    if (groups.length < 2) return { degenerate: true };
    return { groups, score: outcomeSpread(groups) * legibility(groups.length) };
  }

  if (signal.kind === 'outcome') {
    const groups = outcomeGroups(rows, signal.dimension, signal.column, signal.event, { kind: signal.outcomeKind });
    if (groups.length < 2) return { degenerate: true };
    return { groups, score: outcomeSpread(groups) * legibility(groups.length) };
  }
...
/**
 * How few points a trend needs before it stops being a trend.
 *
 * Two is a pair of numbers with a line between them, and the line is the
 * chart's own invention.
 */
const MIN_TREND_POINTS = 4;

/** The ISO prefix each granularity cuts, and what to call the axis. */
const TIME_GRAINS = [
  { prefix: 4, alias: 'Year' },
...
/**
 * Which period to group a date column by.
 *
 * The rule was years past eight of them and months otherwise, with nothing
 * below a month. A sensor stream covering thirty-four days at minute
 * resolution therefore drew two points — a whole January and four days of
 * February — and the finding read "Record Count trended down from 2026-01 to
 * 2026-02, a 88.0% decrease". The decrease was February not having happened
 * yet.
 *
 * So the grain is chosen by counting what each one would actually draw, and
 * taking the coarsest that still has enough points to be a line. Coarsest
 * first, because a year of daily orders is a story about months, not about
 * three hundred and sixty-five days.
 */
function timeGrain(rows, column) {
  const step = Math.max(1, Math.floor(rows.length / 4000));
  const seen = TIME_GRAINS.map(() => new Set());
  for (let i = 0; i < rows.length; i += step) {
    const v = String(rows[i]?.[column] ?? '');
    if (!/^\d{4}-\d{2}-\d{2}/.test(v)) continue;
    TIME_GRAINS.forEach((g, gi) => seen[gi].add(v.slice(0, g.prefix)));
  }
  for (let gi = 0; gi < TIME_GRAINS.length; gi++) {
    if (seen[gi].size >= MIN_TREND_POINTS) return { ...TIME_GRAINS[gi], points: seen[gi].size };
  }
  // Nothing has enough: take the finest reading, and let the evidence tier say
  // how little it rests on.
  const finest = seen.reduce((best, set, gi) => (set.size > seen[best].size ? gi : best), 0);
  return { ...TIME_GRAINS[finest], points: seen[finest].size };
}
 ...
 */
export function planKpis(rows, { provenance = {}, roles = {}, claims = null, profile = null, repeated = null, purpose = null, brief = null } = {}) {
  if (!rows || rows.length === 0) return [];
  // Profiling 250,000 rows costs about two and a half seconds, and this used to
  // happen four separate times in one analysis — here, in the other two entry
  // points, and once in the pipeline — for ten seconds of the twenty-two a
  ...
  if (outcome) {
    const aggregate = outcomeAggregate(outcome);
    /**
     * The headline number for the outcome, computed the way its own kind
     * demands.
     *
     * A share for a flag or a named level; a mean for a continuous outcome.
     * Counting `=== event` on a continuous column matches nothing — every
     * probability is distinct — so the card read "0.0%" on a dataset whose
     * whole subject was that number, which is worse than having no card.
     */
    let value = null;
    if (outcome.kind === 'continuous') {
      ...
      // Averaged over the rows that HAVE a value, not over every row: a column
      // that is a third blank would otherwise report a mean a third too low.
      if (n) {
        const mean = sum / n;
        // `compact` rounds to one decimal, which is right for 89.4K and throws
        // away the number on a probability: an automation risk of 0.53 renders
        // as "0.5", and the headline card of the whole report loses the digit
        // that distinguishes it from an even chance. Sub-unit outcomes — every
        // probability, rate and index there is — keep two.
        value = Math.abs(mean) < 1 ? mean.toFixed(2) : compact(mean);
      }
    } else {
      let hits = 0;
