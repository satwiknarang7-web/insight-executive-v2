...
export function planCharts(rows, { max = 7 } = {}) {
  ...
  if (!p.hasNegatives) {
    ...
    if (sumCol) {
      smallDims.slice(0, 2).forEach((cat, i) => {
        add({
          title: `${pretty(cat)} Share of Records`,
          chart_type: 'donut',
          dimension: cat,
          sql: `SELECT ${br(cat)}, COUNT(*) AS [Count] FROM ${TABLE} GROUP BY ${br(cat)} ORDER BY [Count] DESC LIMIT 6`,
          xAxisKey: cat,
          yAxisKey: 'Count',
          score: 70 - i * 10,
        });
  ...
  contMeasures.slice(0, 2).forEach((m, i) => {
    ...
    // The comparison uses the exact edge; the LABEL is compacted, because
    // "351265-504372" is unreadable on an axis and in a sentence. Bucket labels
    // are display strings only — nothing downstream parses them.
    const lbl = (k) => compact(edge(k));
    const col = br(m);
    const rangeAlias = `${pretty(m)} Range`; // unique per measure so histograms don't collide
    const caseExpr =
      `CASE WHEN ${col} < ${edge(1)} THEN '${lbl(0)}-${lbl(1)}' ` +
      `WHEN ${col} < ${edge(2)} THEN '${lbl(1)}-${lbl(2)}' ` +
      `WHEN ${col} < ${edge(3)} THEN '${lbl(2)}-${lbl(3)}' ` +
      `ELSE '${lbl(3)}+' END`;
    add({
      ...
      sortLabels: [`${lbl(0)}-${lbl(1)}`, `${lbl(1)}-${lbl(2)}`, `${lbl(2)}-${lbl(3)}`, `${lbl(3)}+`],
