...
function selectDiverse(candidates, max, { relatedness = () => 0 } = {}) {
  ...
  const measureOf = (c) =>
    String(c.yAxisKey || '')
      .replace(/^(total|average|avg|sum|count|median|min|minimum|max|maximum|share of|distinct)\s+/i, '')
      .replace(/\s+share$/i, '')
      .trim()
      .toLowerCase();
  const pairOf = (c) => `${dimOf(c)}|${measureOf(c)}`;
...
export function planKpis(rows, { provenance = {}, roles = {} } = {}) {
  ...
  if (kpis.length < 3 && p.dimensions[0]) {
    kpis.push({
      label: `${pretty(p.dimensions[0])} Segments`,
      value: compact(p.cardinality[p.dimensions[0]]),
      trend: 'up',
    });
  }
