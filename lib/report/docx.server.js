...
function barCells(share, negative) {
  ...
  /**
   * Zero gets no bar, and only zero.
   *
   * `MIN_BAR` exists so a value too small to round up to a visible width is not
   * mistaken for nothing. Applied to an actual zero it does the exact opposite:
   * it draws about a millimetre of colour beside the number 0, and a reader
   * skimming the picture rather than the column sees a category that has some
   * of something. The floor is for the tiny, not the absent.
   */
  const filled = share > 0 ? Math.max(MIN_BAR, Math.min(inner, Math.round(share * inner))) : 0;
  const rest = inner - filled;
  const blank = [new Paragraph({ spacing: { before: 0, after: 0 }, children: [new TextRun({ text: ' ', size: 12 })] })];

  // Either part may be the whole track, and a zero-width cell is not a thing
  // Word will draw — so only the parts that exist are built.
  const cells = [];
  if (filled > 0) cells.push(cellFlush(blank, filled, negative ? NEGATIVE : ACCENT));
  if (rest > 0) cells.push(cellFlush(blank, rest, TRACK));

  return new Table({
    layout: TableLayoutType.FIXED,
    width: { size: inner, type: WidthType.DXA },
    columnWidths: [filled, rest].filter((w) => w > 0),
    borders: NO_BORDERS,
...
function chartSeries(chart) {
  ...
  const peak = extent(points.map((p) => Math.abs(p.value))).max;
  return {
    points,
    peak,
    measure: clean(yKey),
          ...
          new TableRow({
            children: [
              cell(
                [
                  cellText(
                    `Filtered: ${clean(analysis.filter.description)}${analysis.filter.rowCount ? ` — ${Number(analysis.filter.rowCount).toLocaleString()} of ${Number(rowCount || 0).toLocaleString()} rows` : ''}.`,
                    { size: 18 }
                  ),
                ],
