...
export function originOf(request) {
  /**
   * An explicit origin wins, because it is the only one nobody can forge.
   *
   * The browser launched below is handed the caller's cookies and told to
   * navigate here, so whatever this returns is a URL this server sends a
   * session to. Every other source below is a request header.
   */
  const configured = process.env.REPORT_ORIGIN || process.env.NEXT_PUBLIC_SITE_URL;
  ...
  /**
   * The scheme the request actually arrived on, not the one the build mode
   * implies.
   *
   * This was `NODE_ENV === 'development' ? 'http' : 'https'`, which is a
   * statement about how the code was built and not about how it is served. An
   * app behind a TLS-terminating proxy — Replit, a container, anything with a
   * load balancer, or plain `next start` — listens on HTTP, so the renderer
   * was sent to `https://<internal-host>` and Chrome refused the connection:
   * `net::ERR_SSL_PROTOCOL_ERROR`, reported to the reader as "Failed to
   * generate PDF". Every deployment that does not itself terminate TLS on the
   * exact host and port in the Host header could not produce a report at all.
   *
   * `x-forwarded-proto` is what those proxies set, and it is the right answer
   * where it exists: the reader's connection really was HTTPS even though ours
   * is not. Where it does not exist we are the edge, and `nextUrl` knows what
   * scheme we are serving.
   */
