...
async function readMarker(supabase, userId) {
  ...
  if (error && !absent(error)) throw error;
 ...
 *
 * `now()` is the database's clock rather than the caller's: a browser whose
 * clock runs fast would otherwise mark the next few minutes of shares read
 * before they arrived. Supabase has no way to ask for `now()` in an upsert
 * value, so the timestamp is omitted and the column default supplies it on
 * insert — and on update the default does not apply, so it is sent explicitly
 * from the server's clock, which is the nearest honest thing.
 */
export async function markNotificationsSeen() {
  ...
  if (error) {
    if (absent(error)) return { seenAt: null };
    throw error;
  }
  return { seenAt };
}
