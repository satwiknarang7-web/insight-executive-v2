...
/**
 * How long the question card waits for a model's reading of the table. Longer
 * than the semantics pass, because it is weighing a list of questions rather
 * than naming a few columns — and short enough that a stalled provider leaves
 * the reader with the catalogue's list, which is complete on its own.
 */
const QUESTIONS_DEADLINE_MS = 20000;
...
export function DatasetProvider({ children }) {
  ...
  const analyze = useCallback(
    async ({ focus = null, maxCharts = null, includeVoid = includeVoidRef.current, prepare = true, questions = null } = {}) => {
      ...
      try {
        ...
        /**
         * No purpose pass and no composed deck any more.
         *
         * Both fed the playbook planner: `purpose` ranked its columns and
         * `/api/compose` had a model write whole charts that the playbook then
         * topped up. A report is now built from questions (phase 2 onwards of
         * docs/design/question-first-reports.md), and a model shapes it by
         * proposing questions on the card (/api/questions) that go through the
         * same compiler as every other — so neither call has anything left to
         * decide, and the two model calls per run are saved. The routes go in
         * phase 5.
         */
        const purpose = null;
        const composed = [];
  ...
  /**
   * What the loaded table can answer, before anything is built — for the
   * question card.
   *
   * The catalogue's questions first, from the worker's rows. Then, where a
   * model is available (the reader's own key, or the deployment's on Pro), its
   * reading: the same briefing the semantics pass sends, plus the catalogue's
   * list, to /api/questions — and its answer back to the worker, which checks
   * every pick and every new question against the rows before the card shows
   * one (lib/modelQuestions.js). No model, a slow one, or nothing usable, and
   * the card has the catalogue's list, which is complete on its own.
   */
  const suggestQuestions = useCallback(async () => {
    const found = await call('suggest', {});
    const loaded = datasetRef.current;
    const profile = loaded?.profile || null;
    const values = loaded?.vocabulary ? valuesBriefing({ vocabulary: loaded.vocabulary, profile }) : claimsBriefing({ profile });
    const answer = await withDeadline(
      fetch('/api/questions', {
        ...
        body: JSON.stringify({
          ...
          rowCount: loaded?.rowCount || null,
          grain: found?.grain || null,
          catalogue: (found?.questions || []).map((q) => ({ id: q.id, text: q.text })),
        }),
      }).then((r) => (r.ok ? r.json() : null)),
      QUESTIONS_DEADLINE_MS
    );
    if (!answer?.proposal) return { ...found, fromModel: false };
    try {
      const checked = await call('acceptQuestions', { proposal: answer.proposal });
      return { ...found, ...checked };
    } catch {
      return { ...found, fromModel: false };
    }
  }, []);
