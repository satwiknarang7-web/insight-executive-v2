...
import { buildStoryboard, narrationRequestFor } from '../storyboard';
import {
  updateSlide,
  removeSlide,
  createSlide,
...
/**
 * Longer than the passes above it, because it is writing a dozen queries rather
 * than naming a few columns — and still short enough that a stalled provider
 * cannot hold a finished analysis hostage. The deck it would have composed is
 * not better than the deck the planner builds in the meantime.
 */
const COMPOSE_DEADLINE_MS = 20000;
...
export function DatasetProvider({ children }) {
  ...
  const analyze = useCallback(
    async ({ focus = null, maxCharts = null, includeVoid = includeVoidRef.current, prepare = true, questions = null } = {}) => {
      ...
      try {
        ...
        /**
         * What the file is for, asked before anything is planned.
         *
         * The one question the planner never had an answer to. Without it,
         * dimensions were ranked by cardinality plus a list of English business
         * words and measures by the width of their numeric range — so a
         * comparison of subscription plans led with a context window counted in
         * tokens, because that column had the widest range in the file.
         *
         * On the critical path for the same reason as the unit claims: it
         * decides what is worth charting, and by the time a background pass ran
         * the charts would already exist. Same short deadline, same cheap
         * failure — no provider, no answer, or nothing that survived checking,
         * and the planner ranks on statistics exactly as it did before.
         */
        const purpose = await withDeadline(
          fetch('/api/purpose', {
            method: 'POST',
            headers: modelHeaders({ 'Content-Type': 'application/json' }),
            body: JSON.stringify({
              ...values,
              fileName: datasetRef.current?.fileName || null,
              rowCount: datasetRef.current?.rowCount || null,
            }),
          })
            .then((r) => (r.ok ? r.json() : null))
            .then((body) => body?.purpose || null),
          SEMANTICS_DEADLINE_MS
        );
        ...
        /**
         * And then let the model choose the charts.
         *
         * The last pass before planning, and the only one that decides what the
         * deck is OF rather than what may be done with a column. It is given
         * the schema, the purpose the pass above settled, and a few whole rows;
         * it returns chart specs with their SQL, already checked against the
         * real columns by the route.
         *
         * Deadlined like the others, and failing costs nothing: no provider, a
         * slow one, or nothing that survived checking, and `composed` is empty
         * — which `runAnalysis` reads as "plan the deck the way you always
         * have". Where it does answer, the planner still tops the deck up, so a
         * model that returns three usable charts does not leave a deck of
         * three.
         */
        const composed = await withDeadline(
          fetch('/api/compose', {
            ...
            headers: modelHeaders({ 'Content-Type': 'application/json' }),
            body: JSON.stringify({
              schema: datasetRef.current?.schema || null,
              columns: datasetRef.current?.columns || [],
              rowCount: datasetRef.current?.rowCount || null,
            ...
            .then((r) => (r.ok ? r.json() : null))
            .then((body) => (Array.isArray(body?.charts) ? body.charts : [])),
          COMPOSE_DEADLINE_MS
        );
         ...
         * steer anything.
         *
         * Distinct from the purpose pass above, which ranks what is worth
         * measuring and breaking out. This one names the dependent variable —
         * the column the rest of the deck is context FOR — and that question
         * was being answered by a list of about forty English nouns, which is
         * why a file of occupations scored for automation risk had no outcome
         * at all.
         *
        ...
        const result = await call(
          'analyze',
          {
            ...
            includeVoid,
            briefProposal,
            // The questions picked on the card. Null: the catalogue's
            // recommended set, which is what "choose for me" means.
            questions,
          },
        ...
        const local = {
          ...
          analystEdits: [],
          // What this report answers, and what else the table could have been
          // asked — kept with the analysis so it is saved with it and "Change
          // questions" opens on what was chosen.
        ...
        /**
         * Background narrative pass, briefed on the finished deck.
         *
         * `result.narrationRequest` describes the charts as the planner first
         * proposed them. By this line the repair pass, the sceptic, the
         * analyst's rewrite and the prepared measures have all been over it,
         * so that draft names slides that are no longer here and calls the
         * surviving ones by headings nobody will read. Writing from it gave
         * pages whose title and opening sentence disagreed about what the
         * chart was.
         *
         * `narrationRequestFor` rebuilds the briefing from `finished` — the
         * deck that was just painted and saved — so the prose is written about
         * the report that exists.
         */
        const briefFinal = narrationRequestFor(result, finished.storyboard);
        if (!briefFinal) {
          setNarrating(false);
          return finished;
        }
        ...
        fetch('/api/narrate', {
          method: 'POST',
          headers: modelHeaders({ 'Content-Type': 'application/json' }),
          body: JSON.stringify(briefFinal),
        })
          .then((r) => (r.ok ? r.json() : null))
          .then((narrative) => {
            ...
            // Rebuilt from the same charts the briefing described. The
            // analyst's edits are reapplied below, so a heading it rewrote
            // survives the round trip rather than reverting to the planner's.
  ...
  /** Put a previous board back, whole. */
  ...
  /**
   * What the loaded table can answer, before anything is built: the catalogue's
   * questions, recommended ones marked, and what one row is. For the question
   * card. Reads the worker's rows; sends nothing anywhere.
   */
  const suggestQuestions = useCallback(() => call('suggest', {}), []);
