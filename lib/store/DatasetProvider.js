...
import { reviewDeck, applyRepairs, rewriteTargets, fallbackRewrites, openQuestions } from '../deckRepair';
  ...
  const analyze = useCallback(
    async ({ focus = null, maxCharts = null } = {}) => {
      ...
      try {
        ...
        const floor = fallbackRewrites(audit, repaired.storyboard).filter((o) => !rewritten.has(o.id));
        const allOps = [...repaired.applied, ...ops, ...floor];
        analystOrderRef.current = ops.find((o) => o.op === 'reorder')?.order || null;

        const finished = {
          ...local,
          storyboard: applyEdits(repaired.storyboard, [...ops, ...floor]),
          kpis: repaired.kpis,
          ...
          critique: [...(local.critique || []), ...openQuestions(audit)],
          analystEdits: allOps,
        };
        ...
        fetch('/api/narrate', {
          ...
          .then((narrative) => {
            ...
            const next = {
              ...
              critique: live?.critique || result.critique || [],
