  ...
  const analyze = useCallback(
    async ({ focus = null, maxCharts = null } = {}) => {
      ...
      try {
         ...
         */
        const claims = await withDeadline(
          fetch('/api/semantics', {
            method: 'POST',
            headers: modelHeaders({ 'Content-Type': 'application/json' }),
            body: JSON.stringify({
              ...claimsBriefing({ profile: datasetRef.current?.profile || null }),
              detected: {},
            }),
          })
            .then((r) => (r.ok ? r.json() : null))
            .then((body) => body?.claims || null),
          SEMANTICS_DEADLINE_MS
        );

        const result = await call(
          'analyze',
          { focus, maxCharts, claims },
         ...
         *
         * Same shape as the narration below it — the deck is already on screen
         * with the questions a rule could find, and this widens the list if a
         * provider answers. It is shown the columns and the chart titles and no
         * values at all, so it has no figure it could quote.
         *
         * Deliberately not awaited and deliberately not fatal: an analysis is
         * finished without it.
         */
        fetch('/api/critique', {
          method: 'POST',
        ...
        /**
         * Background editing pass: the deck as an analyst would leave it.
         *
         * Unlike every other model call here, this one changes the artefact
         * rather than describing it — headings, chart types, the order the
         * story is told in. It is narrow by construction: the briefing carries
         * each slide's legal chart types, computed here from the rows, and the
         * agent picks from that list or picks nothing. It is shown no values,
         * so a digit in a heading it writes is invented and dropped.
         *
         * Its changes land through `updateSlide`, exactly like a change made by
         * hand — recorded in `slide.edits`, shown as modified, undoable, and
         * carried across a re-run.
         */
        ...
        fetch('/api/analyst', {
          ...
          .then((r) => (r.ok ? r.json() : null))
          .then((body) => {
            // Checked again here, against the same briefing, even though the
            // route already checked. The route is the gatekeeper; this is the
            // point of application, and an operation that edits the user's deck
            // should not be one component's correctness away from landing.
            const ops = acceptEdits(body?.edits, { slides: briefing.slides, profile: datasetRef.current?.profile || null });
            const live = analysisRef.current;
            if (!ops.length || !live) return;
            analystOrderRef.current = ops.find((o) => o.op === 'reorder')?.order || null;
            const next = {
              ...live,
              storyboard: applyEdits(live.storyboard, ops),
              analystEdits: ops,
            };
            setAnalysis(next);
        ...
        fetch('/api/narrate', {
          ...
          .then((narrative) => {
            ...
            const next = {
              ...
              critique: result.critique || [],
              // Same reason as the questions above: the narration rephrases
              // findings, it does not re-edit the deck, so what the editing
              // agent changed is still what it changed.
              analystEdits: live?.analystEdits || [],
              narrated: true,
            };
            setAnalysis(next);
            call('saveAnalysis', next).catch(() => {});
