...
import { idbGet, idbSet, idbDel, KEYS } from './idb';
...
export function DatasetProvider({ children }) {
  ...
  // Measures are stored beside the session rather than inside the analysis, so
  // that re-running the dashboard — which replaces every slide — leaves the
  // calculations the user defined by hand untouched.
  useEffect(() => {
    let cancelled = false;
    idbGet(KEYS.measures)
      .then((saved) => {
        if (!cancelled && Array.isArray(saved)) setMeasures(saved);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const commitMeasures = useCallback((next) => {
    ...
    idbSet(KEYS.measures, next).catch(() => {});
    return next;
  }, []);
  ...
  const restoreAnalysis = useCallback(async (payload) => {
    ...
    if (Array.isArray(payload.measures)) {
      ...
      idbSet(KEYS.measures, payload.measures).catch(() => {});
    }
    return restored;
  }, []);
