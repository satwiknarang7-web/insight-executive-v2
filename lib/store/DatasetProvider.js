...
export function DatasetProvider({ children }) {
  ...
  const ingestDocument = useCallback(
    async (file) => {
      ...
      setStatus('ingesting');
      resetProgress('ingest');
      pushProgress('ingest', { stage: 'Reading the document', percent: 15 });
      ...
      try {
        ...
        if (!table?.rows?.length) throw new Error('No table could be read in that document.');

        pushProgress('ingest', {
          stage: 'Cleaning rows',
          percent: 55,
          log: `${table.rows.length} rows read${table.uncertain?.length ? `, ${table.uncertain.length} cells marked unsure` : ''}`,
        });
        ...
        const summary = await call(
          'ingestRemote',
          {
            ...
            sourceLabel: file.name || table.label,
          },
          { onProgress: (p) => pushProgress('ingest', p) }
        );
        setDataset(summary);
  ...
  const analyze = useCallback(
    async ({ focus = null, maxCharts = null, includeVoid = includeVoidRef.current, prepare = true } = {}) => {
      ...
      try {
         ...
         * of steps somebody wrote is their decision about the data, and a
         * model's should not be appended to it unasked. The model reads the
         * columns and their vocabulary and says what it would derive, split,
         * band and name. The part that only ADDS columns goes into effect now,
         * so the planner below sees the margin and the month; the part that
         * would remove or change anything is offered in Explore with its
         * reason. Its measures are saved and go on the dashboard as cards.
         *
        ...
        if (prepare && loaded && unshaped && preparedForRef.current !== loaded.ingestedAt) {
          ...
          const proposal = await withDeadline(suggestPreparation({ focus }), PREPARE_DEADLINE_MS);
          if (proposal && (proposal.steps.length || proposal.measures.length)) {
            prepared = await applyPreparation(proposal);
          }
        }
        ...
        const result = await call(
          'analyze',
          { focus, maxCharts, claims, voidClaim, includeVoid },
    ...
    [pushProgress, resetProgress, endProgress, suggestPreparation, applyPreparation, preparedExtras]
  );
