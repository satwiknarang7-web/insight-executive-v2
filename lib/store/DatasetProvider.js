...
/** How long a new table waits for the model's preparation before going on without it. */
...
/**
 * The worker's progress for an ingest, one step short of done.
 *
 * The worker says Ready when the rows are loaded, but preparation still runs
 * after it, here. Its Ready is held back — the log line kept, the stage turned
 * into the preparation step — and the provider says Ready itself once the
 * table is prepared.
 */
const holdReady = (p) => (p?.stage === 'Ready' ? { ...p, stage: 'Preparing the data', percent: 96 } : p);
...
export function DatasetProvider({ children }) {
  ...
  // The preparation pass, which is defined below the ingest actions that run it.
    ...
    async ({ files, file, text, fileName }) => {
      ...
      try {
        const loaded = await call(
          ...
          { onProgress: (p) => pushProgress('ingest', holdReady(p)) }
        );
        const summary = (await prepareRef.current?.(loaded, run)) || loaded;
        if (run !== runRef.current) return summary;
        setDataset(summary);
        setStatus('ready');
        endProgress();
        return summary;
  ...
  /**
   * The ten minutes an analyst spends on a new table before opening the chart
   * menu, run as part of loading it.
   *
   * Two sources, one path. `deriveSteps` reads the vocabulary the worker has
   * already computed and proposes what the values justify — a month out of a
   * daily date, a compound field split, a continuous number banded — with no
   * key. The model, when one is reachable, reads the same vocabulary and adds
   * what a rule cannot see, such as margin from revenue and cost. Only the
   * steps that ADD a column go into effect (see `applyPreparation`); anything
   * that would filter, drop or change data is offered in Transform data.
   *
   * Before the dashboard is built, because a column added afterwards is one no
   * chart used; deadlined, because a slow provider must not hold the table up,
   * and the table as it arrived is a fine thing to analyse. Returns the dataset
   * as it stands afterwards.
   */
  const prepareLoaded = useCallback(
    async (loaded, run) => {
      if (!loaded?.columns?.length || (loaded.transforms || []).length) return loaded;
      datasetRef.current = loaded;
      const fromModel = await withDeadline(suggestPreparation(), PREPARE_DEADLINE_MS);
      if (run !== runRef.current) return loaded;
      const proposal = preparationFor(fromModel, {
        columns: loaded.columns,
        derived: deriveSteps({ columns: loaded.columns, profile: loaded.profile || null, vocabulary: loaded.vocabulary || null }),
      });
      if (proposal.steps.length) {
        const record = await applyPreparation(proposal, { supersedesAnalysis: false });
        // A step that would not run is reported in Transform data; the table
        // as it arrived is still loaded, so it is not an error for the upload.
        if (record.failure) setError(null);
      }
      pushProgress('ingest', { stage: 'Ready', percent: 100 });
      return datasetRef.current || loaded;
    },
    [suggestPreparation, applyPreparation, pushProgress]
  );
  useEffect(() => {
    prepareRef.current = prepareLoaded;
  }, [prepareLoaded]);
