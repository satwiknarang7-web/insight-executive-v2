 ...
 * `analystPlanner`, and each answered a narrower question than the one a chart
 * needs answered: not "is this a rate" but "which aggregations of this column
 * mean anything, and across what". Nothing here is used by the planner yet;
 * phase 2's chart compilers are built on it. It is scored against the corpus
 * truth by `eval/scorecard.mjs`.
 *
 *   {
...
function typeColumns(rows, temporal = []) {
...
function detectGrain(rows, columns, long) {
  ...
  if (time) {
    ...
    if (columns[time].distinct === n) return { kind: 'event', key: null, time, entity: null, why: `one row per ${time}: a series` };
    ...
    return { kind: 'event', key: null, time, entity: null, why: `rows in time by ${time}, with no identity beyond it` };
  }
