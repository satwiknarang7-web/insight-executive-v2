...
const median = (xs) => {
  // A typed array sorts numerically without calling back into a comparator,
  // several times faster on the column-sized lists this is given.
...
/**
 * The most rows a judgement about typical sizes reads.
 *
 * Whether a measure's size differs tenfold between levels, or where the middle
 * of a column sits, is a question about typical values, and a sample of
 * 25,000 rows answers it as well as all of them — the same cap as
 * lib/chartSignals.js. What needs every row still reads every row: types,
 * distinct counts, whether a column identifies a row, and which single value
 * is the most extreme.
 *
 * Not every tenth row. A table that cycles through its levels — a panel sorted
 * by date, ten stores per day — puts the same store on every tenth row, and a
 * stride would read one store as the whole table. Each row is kept with the
 * same chance instead, from a generator seeded the same way every time, so
 * one file is always read the same way and no period can line up with it.
 */
export const SAMPLE_ROWS = 25000;
export const sampleOf = (xs, cap = SAMPLE_ROWS) => {
  ...
  const keep = cap / xs.length;
  const out = [];
  for (const x of xs) if (next() < keep) out.push(x);
  return out;
};
...
export function outlierInfluence(values) {
  ...
  // The middle and the typical deviation from a sample; the extreme value
  // itself from every row, because a sample would usually step over it.
  const typical = sampleOf(values);
  const med = median(typical);
  const devs = values.map((v) => Math.abs(v - med));
  let worst = 0;
  for (let i = 1; i < devs.length; i++) if (devs[i] > devs[worst]) worst = i;
  const mad = median(sampleOf(devs));
  const second = extent(devs.filter((_, i) => i !== worst)).max;
...
function typeColumns(rows, temporal = [], labels = []) {
  ...
  for (const c of names) {
    // One pass per column, keeping only counts: on a wide file of a quarter of
    // a million rows, an array per column per check was most of the cost.
    let filled = 0;
    let nums = 0;
    let min = Infinity;
    let max = -Infinity;
    let integer = true;
    let yearLike = true;
    // Numbers are counted as numbers: turning a quarter of a million prices
    // into strings only to count them was the single largest cost here.
    const seen = new Set();
    ...
    for (const r of rows) {
      ...
      if (v < min) min = v;
      if (v > max) max = v;
      if (!Number.isInteger(v)) integer = yearLike = false;
      else if (v < 1900 || v > 2100) yearLike = false;
    }
    // As before, 5 and "5" are one value.
    let distinct = seen.size + seenNums.size;
    if (seen.size && seenNums.size) for (const n of seenNums) if (seen.has(String(n))) distinct--;
    const numeric = filled > 0 && nums / filled >= 0.95;
    stats[c] = { min, max, integer };
    let type;
    if (distinct <= 1) type = 'constant';
    else if (temporal.includes(c)) type = 'time';
    else if (numeric && yearLike && distinct <= 200) type = 'time';
    else if (numeric && labelled.has(c)) type = 'category';
    else if (numeric) type = 'number';
    if (type) {
      out[c] = { type, distinct, filled };
      continue;
    }
    const values = rows.map((r) => r?.[c]).filter(present);
    if (values.every((v) => /^https?:\/\//i.test(String(v)))) type = 'url';
  ...
  NUMBERS.set(out, stats);
  return out;
}

/**
 * Each column's range and whether it is all whole numbers, over its numeric
 * values, kept beside the column types rather than in them so the model's
 * shape does not change. Read back by the checks that used to rescan the rows
 * for exactly this.
 */
const NUMBERS = new WeakMap();
const numbersOf = (rows, columns, c) => {
  const known = NUMBERS.get(columns)?.[c];
  if (known) return known;
  const vs = rows.map((r) => r?.[c]).filter(isNum);
  return { ...extent(vs), integer: vs.every(Number.isInteger) };
};
...
function detectLong(rows, columns) {
  ...
  for (const by of labels.filter((c) => columns[c].type !== 'time' && columns[c].distinct <= 30)) {
    ...
    for (const value of numeric) {
      ...
      const spread = medians.length >= 2 ? extent(medians).max / extent(medians).min : 1;
...
function detectResponse(rows, columns) {
  const scales = Object.keys(columns).filter((c) => {
    ...
    const { min, max, integer } = numbersOf(rows, columns, c);
    return integer && max - min <= 10 && columns[c].distinct <= 11;
  });
  if (scales.length < 4) return null;
  // One shared scale: most of them run over the same range.
  const ranges = scales.map((c) => {
    const { min, max } = numbersOf(rows, columns, c);
    return `${min}-${max}`;
  });
  const common = extent([...new Set(ranges)].map((r) => ranges.filter((x) => x === r).length)).max;
...
function scaleFamily(rows, columns) {
  ...
  for (const c of Object.keys(columns)) {
    ...
    const { min: lo, max: hi } = numbersOf(rows, columns, c);
    if (lo < 0) continue;
...
function detectScopes(rows, columns, measures) {
  ...
  //
  // Every pair of measures under every split is a lot of pairs — thirty
  // measures and a dozen splits is ten thousand passes over the rows — and a
  // conversion needs one of the pair to swing tenfold between levels, which
  // almost no measure does. So each measure's groups and swing are found once
  // per split, and a pair where neither side swings that far is never read.
  const conversions = [];
  for (const s of splits) {
    const grouped = new Map(measures.map((m) => [m, groupValues(rows, s, m)]));
    const swings = new Map(measures.map((m) => {
      const ms = [...grouped.get(m).values()].map((xs) => Math.abs(median(xs))).filter((x) => x > 0);
      return [m, ms.length ? extent(ms).max / extent(ms).min : 1];
    }));
    const swing = (col) => swings.get(col);
    for (const a of measures) {
      for (const b of measures) {
        if (a === b) continue;
        if (swing(a) < 10 && swing(b) < 10) continue;
        ...
        const moving = [...grouped.get(a).values()].filter((xs) => {
        ...
        if (!steady || extent(centres).max / extent(centres).min < 3) continue;
  ...
  for (const s of splits.filter(unitLike)) {
    for (const m of measures.filter((x) => MONEY_NAME.test(x))) {
      ...
      const spread = extent(ms).max / extent(ms).min;
...
export function buildTableModel(rows, { temporal = [], labels = [], notSummed = {} } = {}) {
  ...
  const { scopes, why: scopeWhy } = detectScopes(sampleOf(rows), columns, measureNames);
