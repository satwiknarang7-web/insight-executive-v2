...
/**
 * The strip at the foot of a truncated card, holding "+N more columns".
 *
 * It is part of the card's height rather than a label painted into the bottom
 * padding, which is what it used to be: eight pixels of padding is not a row,
 * so the line sat on top of the last column name and both became unreadable.
 * A card that has something to say at the bottom has to be taller.
 */
export const FOOTER_H = 17;

/** Is this card hiding columns? */
export const isTruncated = (columnCount) => columnCount > MAX_ROWS;
...
export function cardHeight(columnCount) {
  ...
  return HEADER_H + shown * ROW_H + CARD_PAD * 2 + (isTruncated(columnCount) ? FOOTER_H : 0);
}

/** The baseline of the "+N more columns" line, clear of the last column row. */
export function footerBaseline(columnCount) {
  return cardHeight(columnCount) - CARD_PAD - 4;
}
