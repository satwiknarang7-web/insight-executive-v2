 ...
 */
const THEMES = [
  { key: 'default', name: 'Signal', order: ['aqua', 'orange', 'blue', 'yellow', 'magenta', 'green', 'violet', 'red'] },
  { key: 'ocean', name: 'Ocean', order: ['blue', 'orange', 'aqua', 'yellow', 'magenta', 'green', 'violet', 'red'] },
...
/**
 * The colour for the `index`th *bar* of a chart that paints one colour per bar.
 *
 * This is the one place a repeat is allowed, and the reason is what colour is
 * doing. In a legend, a donut or a stack, colour is the only thing tying a mark
 * to its name, so spending one twice makes the chart say two things are the
 * same — `seriesColor` refuses, and should keep refusing. On a bar chart
 * coloured per category the name is printed under the bar: the colour is
 * decoration, and the ninth bar going grey, then the tenth going the same grey,
 * is the one arrangement that *does* claim a relationship that is not there.
 *
 * So it cycles, a lap at a time, with each lap stepped away from the last, so
 * bar nine is recognisably not bar one. The second lap moves *away* from the
 * surface — lighter on a dark background, darker on a light one — because that
 * is the direction the hues have room in and the direction that keeps their
 * 3:1 contrast rather than spending it. The third comes back the other way.
 * Three laps of eight, and then the steps repeat; past twenty-four bars the
 * axis label is doing all the work anyway.
 */
const LAP_STEPS = [0, 0.26, -0.22];

export function categoryColor(palette, index, mode = 'dark') {
  const colors = palette?.length ? palette : paletteFor('default', mode);
  const i = Math.max(0, Math.trunc(index) || 0);
  const base = colors[i % colors.length];
  const lap = Math.floor(i / colors.length) % LAP_STEPS.length;
  const step = LAP_STEPS[lap];
  if (!step) return base;
  // Away from the surface: towards white on dark, towards black on light.
  return shade(base, mode === 'light' ? -step : step);
}

/**
 * Move a hex colour towards white (`amount` > 0) or black (`amount` < 0).
 *
 * A blend rather than an HSL lightness change: blending keeps the two ends of
 * the scale behaving the same way, where HSL compresses everything near the
 * top and turns a step into no visible change at all.
 */
function shade(hex, amount) {
  ...
  const channel = (offset) => {
    ...
    const mixed = Math.round(from + (target - from) * weight);
