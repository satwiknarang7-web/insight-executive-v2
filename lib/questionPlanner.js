...
const role = (name, lead, spoken = true) => ({ role: name, spoken, re: new RegExp(lead + UNTIL, 'i') });

/**
 * `spoken` marks the phrasings that can only be someone naming a column.
 *
 * It decides what happens when the phrase matches nothing in the data. "Which
 * Payment_Mode has the highest Quantity?" over a table with no such column was
 * answered with one card holding the total quantity — the grouping was dropped
 * in silence and the number that came back was 100% of itself. So a phrase from
 * a `spoken` pattern that names no column stops the read and is named in the
 * refusal.
 *
 * "per" is not one of them: "average per order" is how an average is asked for,
 * not a grouping, and refusing that would be worse than ignoring it.
 */
const ROLE_PATTERNS = [
  role('row', '\\brows?\\s*[:=]\\s*'),
 ...
 *
 * The question has its own punctuation stripped first, because `\b` counts an
 * underscore as a word character: against the raw text `\bcharge\b` does not
 * match inside "monthly_charge", and nor does `\bmonthly\b`. So the one
 * phrasing this refused was a question naming a column exactly as the file
 * spells it — including the starter questions this page writes for itself,
 * which are built from the column names and so always spell them that way.
 */
function namedIn(text, column) {
  ...
  // "monthlycharge", written as one word, has no separator left to normalise,
  // so the glued form is tested against a glued haystack — and only that form:
  // run the single words against it and "charge" matches inside "monthlycharge".
  const glued = parts.join('');
  const spaced = parts.join(' ');
  if (new RegExp(`\\b${glued}(?:e?s)?\\b`, 'i').test(hay.replace(/ /g, ''))) return true;
  for (const candidate of [spaced, ...parts]) {
    // Nothing to escape: `parts` was split on everything but a-z0-9.
    const re = new RegExp(`\\b${candidate}(?:e?s)?\\b`, 'i');
 ...
 */
/**
 * Words that follow "by" without naming a column: a period to bucket a date
 * into ("by month"), which `readBucket` reads separately.
 */
const PERIOD_PHRASE = /^(?:the\s+)?(?:day|daily|week|weekly|month|monthly|quarter|quarterly|year|yearly|annual|annually|hour|hourly|date|time|period)s?$/i;

function readGroupings(question, dimensions, columns = []) {
  const found = [];
  const unplaced = [];
  let rest = String(question || '');

  for (const { role, re, spoken } of ROLE_PATTERNS) {
    const match = rest.match(re);
    if (!match) continue;
    const column = matchColumn(match[1], dimensions, { exclude: found.map((f) => f.column) });
    if (!column || !namedIn(match[1], column)) {
      const phrase = clean(match[1]);
      // Not every miss is a mistake. "by month" is a period, and "by revenue"
      // names a column that is a measure rather than something to group by —
      // both were always read elsewhere, and both are left alone here. What is
      // left is a phrase that names nothing in this data at all.
      const known = matchColumn(phrase, columns);
      const namesSomething = PERIOD_PHRASE.test(phrase) || (known && namedIn(phrase, known));
      // The same phrase is re-read by the looser patterns once the strict one
      // has left it in place, so the same miss can arrive more than once.
      if (spoken && phrase && !namesSomething && !unplaced.includes(phrase)) unplaced.push(phrase);
      continue;
    }
  ...
  return { groupings: found, rest, unplaced };
}
...
export function planQuestion(question, context = {}) {
  ...
  const { groupings, rest, unplaced } = readGroupings(text, dimensions, context.columns || []);

  // A grouping was named and there is no such column. Dropping it answers a
  // different, smaller question — usually one ungrouped total — and puts a
  // confident sentence under it, so it is refused by name instead.
  if (unplaced.length) {
    return {
      spec: null,
      error:
        `Nothing here is called “${unplaced[0]}”, so there is nothing to break the answer down by. ` +
        `The columns you can group by are: ${dimensions.slice(0, 8).join(', ')}${dimensions.length > 8 ? '…' : ''}.`,
    };
  }
