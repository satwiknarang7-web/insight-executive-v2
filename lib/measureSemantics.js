...
export function classifyColumns({
  ...
  repeatedAt = {},
  denominatedBy = null,
  // The rows, where a caller has them. Only the value-scale test below needs
  // them, and it simply does not run without them.
  rows = null,
} = {}) {
  ...
  const units = { ...(denominatedBy || {}), ...detectDenomination({ profile, cardinality, rows }) };
  ...
  for (const col of numeric) {
    ...
    /**
     * Near-unique is what CONTINUOUS means, not what identifying means.
     *
     * A measurement taken to two decimal places is distinct on nearly every
     * row by construction. On a long-format indicator export the `value`
     * column had 288 distinct values across 288 rows, was called an identifier
     * on that basis alone, and the classification stopped there — so the
     * evidence a few lines down, that the column holds a GDP on one row and a
     * life expectancy on the next, never got a say and the deck averaged them
     * together.
     *
     * The name is a reliable signal and keeps deciding on its own. Uniqueness
     * is only a signal alongside the shape of an id: whole numbers running
     * through a dense range, which is what a row number or a sequence key
     * looks like and what no measurement looks like. Checked only when the
     * caller passed the rows; without them this is exactly the old rule.
     */
    const sequence = () => {
      ...
      if (nums.length < 8 || !nums.every(Number.isInteger)) return false;
      return Math.max(...nums) - Math.min(...nums) <= distinct * 1.25;
    };
    if (IDENTIFIER_RE.test(col) || (rowCount > 8 && distinct >= 0.95 * rowCount && sequence())) {
      kind = 'identifier';
