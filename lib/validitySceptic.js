 ...
 */
export function passages({ findings = [], storyboard = [] } = {}) {
  const out = [];
 ...
 */
export function checkValidity({ findings = [], storyboard = [], rowCount = 0 } = {}) {
  const out = [];
  ...
  for (const p of passages({ findings, storyboard })) {
    const m = p.metrics || {};
    ...
    if (
      (p.field === 'recommendation' || p.field === 'insight_question') &&
