...
export function checkValidity({ findings = [], storyboard = [], slideZero = null, rowCount = 0 } = {}) {
  ...
  for (const p of passages({ findings, storyboard, slideZero })) {
    ...
    if (
      ...
      // Nobody is clear of the field when several share the top value, however
      // the standard deviations come out — a z-score measured against a field
      // that contains six more copies of the top value is not separation.
      !m.tiedAtTop &&
