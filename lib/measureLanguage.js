...
const PERCENT_OF = /\s+(?:as\s+(?:a\s+)?)?(?:%|percent(?:age)?|share|proportion|fraction)\s+of\s+/i;
/** "the ratio of X to Y". */
...
/**
 * Is the left-hand side of a share split an actual quantity, or just the word
 * that was aggregating the column on the right?
 *
 * "of total" is a share in "shipping cost as a share of total revenue", and is
 * nothing of the sort in "sum of total amount" — where it sits inside the name
 * of the column being asked for. Columns called `Total_Amount` and
 * `Total_Revenue` are everywhere, so this was not an edge case: "sum of total
 * amount" was declined outright, and "total of total amount" came back as
 * SUM([Total_Amount]) / SUM([Total_Amount]) — a measure reading 100%, which is
 * true, useless, and not what anybody asked for.
 */
function isRealOperand(text) {
  let left = String(text || '');
  for (const { re } of AGGREGATE_WORDS) left = left.replace(new RegExp(re.source, 'gi'), ' ');
  // Articles, prepositions and question words are not quantities either: "what
  // share of revenue comes from Electronics" has nothing on the left of the
  // split, and reading "what" as the numerator loses the question.
  const bare = left.replace(/\b(?:the|a|an|of|for|in|my|our|what|which|how|show|give|me|is|are)\b/gi, ' ');
 ...
 *
 * Exported for the step parser, which reads "keep rows where region is West"
 * with the same grammar a measure's trailing "where" uses — one reading of a
 * condition, so the two cannot disagree about what "is not" means.
 */
export function parseFilter(text, ctx) {
...
export function parseMeasurePhrase(phrase, { columns = [], profile = null, measures = [] } = {}) {
  ...
  if (asPercent.length === 2 && isRealOperand(asPercent[0])) {
    percent = true;
    body = `the ratio of ${asPercent[0]} to ${asPercent[1]}`;
  } else if (asShare.length === 2 && isRealOperand(asShare[0])) {
    percent = true;
...
export function exampleMeasurePhrases(profile, { measures = [], sample = [] } = {}) {
  ...
  const candidates = [];
  if (primary) candidates.push(phrased('total', primary));
  if (primary) candidates.push(phrased('average', primary));
  ...
  if (primary && category && value) {
    candidates.push(`${phrased('total', primary)} where ${pretty(category)} is ${value}`);
  }
  ...
  if (primary) candidates.push(phrased('maximum', primary));
...
/**
 * A phrase that does not repeat itself.
 *
 * `total ${pretty('Total_Amount')}` is "total total amount", which reads as a
 * stutter in the one place that is meant to teach people how to phrase things.
 */
function phrased(word, column) {
  const name = pretty(column);
  return name === word || name.startsWith(`${word} `) ? name : `${word} ${name}`;
}
