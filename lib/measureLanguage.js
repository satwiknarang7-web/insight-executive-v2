 ...
 */
export function exampleMeasurePhrases(profile) {
  const numeric = numericColumns(profile);
  // A money column is the one people actually measure, so it leads. Pairing two
  // arbitrary numeric columns produced suggestions like "units minus price",
  // which teach the syntax by way of a calculation nobody would ever want.
  const money = numeric.filter((c) => isCurrencyKey(c)).sort((a, b) => additivity(b) - additivity(a));
  const primary = money[0] || numeric[0];
  // A denominator people divide by is usually a count, not more money.
  const other =
    numeric.find((c) => c !== primary && !isCurrencyKey(c)) || numeric.find((c) => c !== primary);
  const dimension = (profile?.dimensions || []).find((d) => !TEMPORAL_NAME.test(d));

  const out = [];
  if (primary) out.push(`total ${pretty(primary)}`);
  if (primary) out.push(`average ${pretty(primary)}`);
  if (primary && other) out.push(`${pretty(primary)} per ${pretty(other)}`);
  if (money.length > 1) out.push(`${pretty(money[1])} as a percentage of ${pretty(money[0])}`);
  if (dimension) out.push(`number of distinct ${pretty(dimension)}`);
  if (primary && dimension) out.push(`total ${pretty(primary)} where ${pretty(dimension)} is …`);
  return out.slice(0, 5);
}

const TEMPORAL_NAME = /(date|time|year|month|day|quarter|qtr|week)/i;

/**
 * How much sense a column makes to add up.
 *
 * `revenue` totals to something meaningful; `unit_price` totals to a number
 * with no interpretation at all. Both look like money to the formatter, so the
 * examples rank them rather than trusting column order.
 */
function additivity(column) {
  const name = normalize(column);
  let score = 0;
  if (/revenue|sales|amount|profit|margin|spend|cost|value|total/.test(name)) score += 3;
  if (/price|rate|per |unit|avg|average|ratio/.test(name)) score -= 2;
  return score;
}
