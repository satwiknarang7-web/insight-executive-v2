...
const SLIDE_FIELDS = new Set([
  'pageTitle',
  // How much of a dashboard row this finding takes. On the whitelist like any
  // other field a person can change, which is what makes it survive a re-run:
  // `reapplyEdits` carries exactly what is listed here.
  'size',
  // Where it sits on the canvas: {x, y, w, h}. The arrangement is a thing the
  // reader said about their findings — this one is the story, those two are its
  // drivers — and a re-run that rebuilt the board would otherwise throw it away.
  'layout',
  'analystNotes',
  'speech',
  'insight_anchor',
...
const CHART_FIELDS = new Set([
  ...
  'xAxisLabel',
  'yAxisLabel',
  // How a slicer offers its values: every box on show, or a dropdown that opens
  // one. A choice about the tile rather than about the data, which is why it
  // lives beside the palette and not in the query.
  'slicerMode',
]);
...
export function updateKpi(kpis, index, patch = {}) {
  ...
  return list.map((kpi, i) => {
    ...
    // Where it sits on the board. A card is a tile like any other now, so it
    // carries a box and keeps it through a re-run — see `layout` on a slide.
...
export function addKpi(kpis, card = {}) {
  ...
  // An id of its own, because a tile on a canvas is identified rather than
  // counted: the index a card sits at changes the moment one before it is
  // deleted, and its box would follow the wrong card.
  added.id = card.id || `kpi_${Date.now().toString(36)}${list.length}`;
  if (card.layout) added.layout = card.layout;
...
export function reapplyKpiEdits(fresh, previous) {
  ...
  const merged = list
    ...
    .map((k) => {
      ...
      if (!old) return k;
      // The box comes back with the label: a card somebody moved to the corner
      // of the board and then re-ran the analysis on should still be in the
      // corner, and the re-run has just composed a fresh one for it.
      const kept = { ...k, label: old.label, value: old.value, origLabel: String(k.label), edited: true };
      if (old.layout) kept.layout = old.layout;
