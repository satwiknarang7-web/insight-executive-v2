 ...
 * an exposure to hedge" word for word, about different columns, on the same
 * deck. Identical strings are not a judgement call, so the second one is
...
function repeatedAdvice(findings = []) {
  ...
  for (const f of findings || []) {
    const advice = key(f?.recommendation);
...
/**
 * Rows the data itself says did not happen, counted inside every total.
 *
 * A table with an `Order_Status` column holding `Cancelled` and `Returned` is
 * telling you which rows are not revenue. Summing across all of them produces a
 * figure whose name is wrong — not slightly, and not in a way any later
 * sentence can repair, because every share, every ranking and every trend built
 * on that measure inherits it.
 *
 * This only DETECTS. Filtering is a decision about what the customer means by
 * their own measure, and making it silently would be this file deciding what a
 * business counts as revenue. What it earns is first place in the argument: a
 * doubt this size belongs before the claim it undermines, not in a footnote
 * after it.
 *
 * @param {object[]} rows    the analysis view
 * @param {object}   profile columns and their roles
 */
export function contestedBasis(rows = [], profile = null) {
  const dimensions = profile?.dimensions || [];
  const measures = profile?.measures || [];
  if (!rows?.length || !measures.length) return null;

  const column = dimensions.find((d) => STATUS_COLUMN.test(norm(d)));
  ...
  for (const row of rows) {
    ...
    if (!value) continue;
    if (VOID_STATUS.test(value)) {
      voided += 1;
      levels.add(value);
    }
  }
  if (!voided || voided / rows.length < MIN_VOID_SHARE) return null;

  return {
    column,
    levels: [...levels],
    rows: voided,
    sharePct: round((voided / rows.length) * 100),
    measures,
  };
}
 ...
 */
export function argumentSpine({ findings = [], connections = [], contested = null, rowCount = 0 } = {}) {
  const steps = [];
  ...
  if (contested) {
    steps.push({
      ...
      cites: [],
      text:
        `${contested.rows.toLocaleString()} of ${rowCount.toLocaleString()} rows (${contested.sharePct}%) are marked ` +
        `${contested.levels.join(' or ')} in ${contested.column}, and every total below counts them. ` +
        'Decide whether those rows belong in the measure before reading any figure as revenue.',
    });
  }
  ...
  const decision = decisionFrom(ranked, contested);
 ...
 */
function decisionFrom(findings = [], contested = null) {
  if (contested) {
    return {
      role: 'decision',
      cites: [],
      text:
        `Settle the treatment of the ${contested.rows.toLocaleString()} ${contested.levels.join('/')} rows first. ` +
        'Every figure in this report moves when that is decided, so nothing below is worth acting on until it is.',
    };
  }
