  ...
  for (const f of findings || []) {
    ...
    if (seen.has(advice)) {
      out.push({
        ...
        // Pointed, not deleted.
        //
        // Clearing it left a slide that described a bar chart and stopped —
        // the duplicate boilerplate fixed by removing one copy rather than by
        // saying anything. A reader on that slide still has a decision to make;
        // what they do not need is the same paragraph twice.
        repair: {
          op: 'set_text',
          id: String(f?.id ?? ''),
          field: 'insight_question',
          text: `The same decision applies here as on "${seen.get(advice)}" — it is one call about concentration, not two.`,
          why: 'the same instruction is already given on another slide',
        },
      });
      continue;
    }
...
export function argumentBriefing({ steps = [], findings = [] } = {}) {
  ...
  return {
    steps: (steps || []).map((s, i) => ({
      ...
      says: s.text,
      cites: s.cites || [],
      // A statement of what was done to the data is not prose to be improved.
      //
      // The basis step said "25,000 of 250,000 rows (10%) are marked Cancelled
      // or Returned in Order_Status and are excluded from every figure below.
      // The report describes the remaining 225,000 rows." A model rewrote it as
      // the second sentence alone, which passed every check — it quotes only
      // figures the step contains — and left a report whose summary says
      // "the remaining 225,000 rows" with nothing anywhere saying remaining
      // from what. The filter became unreconstructable.
      //
      // `caveats` are already held back from the narrator for this reason. The
      // same rule reaches here.
      fixed: s.role === 'basis',
...
export function acceptArgument(raw, { steps = [] } = {}) {
  ...
  // Steps whose wording is a disclosure rather than a description. A model may
  // read them for context and may not replace them.
  const fixed = new Set((steps || []).map((s, i) => (s.fixed ? String(i + 1) : null)).filter(Boolean));
  const out = [];
  ...
  for (const item of raw) {
    ...
    if (supporting === undefined || fixed.has(cites)) continue;
