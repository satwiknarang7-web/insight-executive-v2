...
export function suggestQuestions(rows, model, { outcome = null } = {}) {
  ...
  if (primary && !long && !entityLike) {
    ...
    for (const f of flows.slice(1, 3)) {
      for (const d of dims.slice(0, 2)) offer({ intent: 'composition', measure: f, by: d, text: `Where does ${name(f)} come from, by ${name(d).toLowerCase()}?` });
    }
  }
  ...
  for (let i = 0; i < pool.length; i++) {
    for (let j = i + 1; j < pool.length; j++) {
      ...
      related.push({ a: pool[i], b: pool[j], r });
    }
  }
  related.sort((x, y) => y.r - x.r);
  related.slice(0, 3).forEach(({ a, b, r }, i) =>
