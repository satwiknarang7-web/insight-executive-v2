...
function ordinal(model, rows, col) {
  ...
  return values.every(Number.isInteger) && Math.max(...values) - Math.min(...values) <= 10;
}
...
function measureSets(model, rows) {
  ...
  // Money first: of the totals a table carries, the one in currency is the one
  // it is usually kept for.
  flows.sort((a, b) => (model.measures[b].unit === 'currency') - (model.measures[a].unit === 'currency'));
...
export function outcomeSpread(rows, outcome, driver, { band = false } = {}) {
  ...
  return { spread: Math.max(...rates) - Math.min(...rates), separates: rates.every((r) => r <= 1 || r >= 99) };
}

/** A two-level yes/no column, read as an outcome: which level is the event. */
function flagOutcome(rows, col) {
...
export function suggestQuestions(rows, model, { outcome = null } = {}) {
  ...
  const seen = new Set();
  const ask = (q) => {
    const id = [q.intent, q.outcome?.column, q.measure, q.cost, q.other, q.by, q.over, ...(q.items || [])].filter(Boolean).join('|');
  ...
  const flagColumns = Object.keys(model.columns).filter((c) => flagOutcome(rows, c));
  ...
  if (long) {
    ...
    for (const d of dims.slice(0, 2)) {
      ask({ intent: 'compare', measure: long.value, by: d, text: `How do ${name(d).toLowerCase()}s compare on each ${name(long.by).toLowerCase()}?` });
    }
  }
  ...
  binaries.forEach((o, n) => {
    ask({ intent: 'outcome-rate', outcome: o, text: `What share are ${name(o.column).toLowerCase()}?` });
    const drivers = [
      ...
      .map((d) => ({ ...d, ...outcomeSpread(rows, o, d.by, { band: d.band }) }))
    ...
    for (const d of drivers.slice(0, n === 0 ? 3 : 1)) {
      ask({ intent: 'outcome-rate', outcome: o, by: d.by, band: d.band, text: `Does ${name(d.by).toLowerCase()} change the ${name(o.column).toLowerCase()} rate?` });
    }
    if (time && n === 0) ask({ intent: 'trend', outcome: o, over: time, text: `How has the ${name(o.column).toLowerCase()} rate moved over time?` });
  });
  ...
  if (continuous) {
    const byExplained = dims
      ...
      .map((d) => ({ d, e: explained(rows, continuous, d) }))
      .filter((x) => x.e > 0)
      .sort((a, b) => b.e - a.e);
    ...
    const partner = Object.keys(model.measures)
      ...
      .map((m) => ({ m, r: Math.abs(correlation(rows, continuous, m)) }))
      .filter((x) => x.r < 0.97)
      .sort((a, b) => b.r - a.r)[0];
  ...
  const pairs = comparable
    ...
    .flatMap((m) => dims.filter((d) => d !== m && !bandingOf(rows, m, d)).map((d) => ({ m, d, e: explained(rows, m, d) })))
    .filter((p) => p.e > 0.02)
    .sort((a, b) => b.e - a.e);
  ...
  if (primary && !long && !entityLike) {
    for (const d of dims.slice(0, 3)) ask({ intent: 'composition', measure: primary, by: d, text: `Where does ${name(primary)} come from, by ${name(d).toLowerCase()}?` });
  }
  ...
  if (!entityLike && dims[0]) {
    ...
    for (const a of counts) {
      for (const b of counts) {
        ...
        if (both.length < 10 || both.some((r) => r[a] > r[b])) continue;
  ...
  for (let i = 0; i < pool.length; i++) {
    for (let j = i + 1; j < pool.length; j++) {
      const r = Math.abs(correlation(rows, pool[i], pool[j]));
      if (r > 0.97 || r < 0.3) continue;
      if (!best || r > best.r) best = { a: pool[i], b: pool[j], r };
    }
  }
  if (best) ask({ intent: 'relationship', measure: best.b, other: best.a, text: `Does ${name(best.a)} move with ${name(best.b)}?` });

  return out.map((q, i) => ({ ...q, recommended: i < RECOMMENDED }));
}
