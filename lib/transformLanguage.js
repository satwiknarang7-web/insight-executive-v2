...
const RULES = [
  ...
  {
    re: /^(?:add|create|derive|compute|calculate|new column)\s+(?:a\s+|an\s+)?(?:new\s+)?(?:column\s+)?(?:called\s+|named\s+)?(.+?)\s*(?:=|:=|as|equal to|equals)\s*(.+)$/i,
    build: (m) => {
      const expr = m[2].trim();
      // A formula mentions its columns in brackets. Anything else is prose
