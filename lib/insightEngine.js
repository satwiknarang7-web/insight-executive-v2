...
function analyzeRanking(series, ctx) {
  ...
  const evidenceOf = evidence({
    ...
    notes: [
      tied ? 'the leader is not separated from the rest of the field' : null,
   ...
   */
  const opener = leadingIsBad(measure)
  ...
  const headline = additive
    ...
    : avg > 0
      ...
      ? top.value / avg >= MIN_MEANINGFUL_MULTIPLE
        ? `${opener}, ${(top.value / avg).toFixed(1)}× the ${val(avg)} average across ${points.length} ${groupNoun(dim.toLowerCase())}.`
        : `${opener}.`
      : `${opener}.`;
  ...
  const parts = [
    say(
      ...
      (isPct
        ? `That is ${compactNum(Math.abs(top.value - avg))} percentage points ${deltaVsAvg >= 0 ? 'above' : 'below'} the ${val(avg)} average across ${points.length} ${groupNoun(dim.toLowerCase())}`
        : `That is ${pct(Math.abs(deltaVsAvg))} ${deltaVsAvg >= 0 ? 'above' : 'below'} the ${compactNum(avg)} average across ${points.length} ${groupNoun(dim.toLowerCase())}`) +
        (gapToSecond !== null
...
function rankingRecommendation({
  tier, concentrated, tied, leader, belowAverage, basis, dim, count = 0, trailer = null,
  measure = null,
}) {
...
function levelStability(ys) {
  const spread = (xs) => {
    ...
    if (!level) return 0;
    return (Math.max(...xs) - Math.min(...xs)) / level;
  };
