...
export function analyzeStoryboard(charts, rawRows = []) {
  ...
  // of them produces it.
  const connections = reconcile(perChart);
  ...
  const riskSource = trendIsMaterial ? trendRisk : concentrated || trendRisk;
...
function reconcile(findings) {
  ...
  if (trend && concentration && out.length < 3) {
  ...
  if (skewed && out.length < 3) {
    ...
    if (averaged) {
      out.push(
        `${prettyKey(subject)} runs long to the ${skewed.metrics.skew === 'right' ? 'high' : 'low'} end, so the averages in "${averaged.title}" sit ${skewed.metrics.skew === 'right' ? 'above' : 'below'} the typical record and the ranking compares means that no ordinary ${lowerFirst(averaged.dimension || 'category')} actually looks like. Compare medians before acting on that order.`
      );
    }
  }
...
function implication(finding, used = new Set()) {
  ...
  if (isNum(share) && share >= CONCENTRATION_PCT) {
    ...
    offer(
      'concentration',
      `${who} carrying ${pct(share)} of ${of} means a bad quarter there is a bad quarter overall — that is a concentration to decide about, not a fact to note.`
    );
  ...
  const fresh = options.find((o) => !used.has(o.kind)) || options[0];
