...
import { leadingIsBad, rankingVerb } from './metricPolarity.js';
...
function analyzeRanking(series, ctx) {
  ...
  const opener = `${top.label} ${rankingVerb(measure)} ${plural(dim.toLowerCase())} on ${measurePhrase(measure)} at ${val(top.value)}`;
  const headline = additive
    ...
    : avg > 0
      ...
      ? top.value / avg >= MIN_MEANINGFUL_MULTIPLE
        ? `${opener}, ${(top.value / avg).toFixed(1)}× the ${val(avg)} average across ${points.length} ${plural(dim.toLowerCase())}.`
        : `${opener}.`
      : `${opener}.`;
  ...
  const parts = [
    say(
      ...
      (isPct
        ? `That is ${compactNum(Math.abs(top.value - avg))} percentage points ${deltaVsAvg >= 0 ? 'above' : 'below'} the ${val(avg)} average across ${points.length} ${plural(dim.toLowerCase())}`
