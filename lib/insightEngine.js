...
function analyzeTrend(series, ctx) {
  ...
  if (perPeriod !== null) {
    detailParts.push(
      `That works out to about ${pct(Math.abs(perPeriod))} ${perPeriod >= 0 ? 'growth' : 'decline'} per period, compounded across ${periods} ${periods === 1 ? 'interval' : 'intervals'}.`
    );
  }
  detailParts.push(
...
export function analyzeStoryboard(charts, rawRows = []) {
  ...
  const riskSource = concentrated || (trend?.metrics.direction === 'declining' ? trend : null);
  ...
  const opportunity =
    perChart.find((f) => notTheRisk(f) && f.metrics.direction === 'rising') ||
    ...
    ranked.find((f) => notTheRisk(f) && !f.metrics.modalBucket && f.metrics.direction !== 'declining');
...
function implication(finding) {
  ...
  if (m.direction === 'rising' || m.direction === 'declining') {
    const rate = isNum(m.perPeriodPct) ? ` at about ${pct(Math.abs(m.perPeriodPct))} per period` : '';
    const recent =
    ...
    return m.direction === 'declining'
      ? `Left alone it keeps falling${rate}, so the question is what changed rather than whether to react.${recent}`
      : `Sustained${rate} that compounds, so targets set off the starting point will be met early and stop meaning anything.${recent}`;
  }
  if (m.direction === 'flat') {
