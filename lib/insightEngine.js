...
function analyzeRanking(series, ctx) {
  ...
  const headline =
    `${top.label} leads ${dim.toLowerCase()} on ${measure.toLowerCase()} at ${compactNum(top.value)}` +
    (leaderShare > 0 ? `, ${pct(leaderShare)} of the total` : '') + '.';

  let detail =
    `That is ${pct(Math.abs(deltaVsAvg))} ${deltaVsAvg >= 0 ? 'above' : 'below'} the ${compactNum(avg)} average across ${points.length} ${dim.toLowerCase()}s` +
...
function analyzeComposition(series, ctx) {
  ...
  const headline = `${top.label} makes up the largest share of ${measure.toLowerCase()} at ${pct(topShare)} of the total.`;
  const detail =
    `The top two ${dim.toLowerCase()}s together account for ${pct(top2Share)} across ${points.length} segments. ` +
    (hhi >= 0.5
...
export function analyzeChart(chart) {
  ...
  } else if (type === 'line' || type === 'area' || isTemporal) {
...
export function analyzeStoryboard(charts, rawRows = []) {
  ...
  const macroInsights = ranked.slice(0, 3).map((f) => f.headline);

  // Strategic scorecard derived deterministically.
  const trend = perChart.find((f) => f.metrics.direction);
  const downtrend = perChart.find((f) => f.metrics.direction === 'declining');
  ...
  const opportunity =
    ...
    ranked.find((f) => isNum(f.metrics.leaderSharePct) || isNum(f.metrics.dominantSharePct)) ||
...
function looksTemporal(labels) {
  ...
  const hits = labels.filter((l) => {
    ...
    // Reject numeric ranges / open-ended buckets ("1852-2578", "3303+").
    if (/[+]/.test(s) || /^\d+\s*-\s*\d+$/.test(s)) return false;
    return (
