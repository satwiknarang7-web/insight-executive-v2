...
import { DEFAULT_SLICER_MODE } from './chartSpecs.js';
...
/**
 * What the card around a filter costs before any of its values are drawn.
 *
 * Every tile on this board is drawn inside a card that has its own padding, its
 * own type label and its own title, and a height planned without them is a
 * height the tile does not get: a filter allotted 96px had about twenty left
 * once the card had taken its share, so the control it exists to show was cut
 * off by the bottom edge. The card is the same card everywhere, so its chrome
 * is a constant here rather than a guess in the tile.
 *
 * Measured tight. Every pixel a filter takes off the top of the page is a pixel
 * the charts divide between them, and a strip of two filters that reached a
 * fifth of the way down the board was a fifth of the board spent on two words
 * and a dropdown.
 */
const SLICER_CHROME = 84;

/** And the search field, which appears once there are more values than the eye can scan. */
const SLICER_SEARCH = 34;
const SEARCHABLE = 8;

const SLICER_ROW = 26;
const listHeight = (values) => {
  const rows = Math.min(Number(values) || 4, 12);
  return SLICER_CHROME + (rows > SEARCHABLE ? SLICER_SEARCH : 0) + SLICER_ROW * rows;
};

/** A title and one line that opens the rest, and nothing else. */
const DROPDOWN_HEIGHT = SLICER_CHROME + 36;

/** Under this a chart row is readable but no longer comfortable. */
const COMFORTABLE_ROW = 180;

/**
 * How the filters offer their values, and what that costs the page.
 *
 * A list is better to use — every value visible, one click to tick one — and on
 * a page of fixed height it is paid for out of the charts. Three values across
 * the full width of a board is a hundred and eighty pixels holding three
 * checkboxes, and the charts underneath lose that height between them.
 *
 * So the list is kept while the charts can still breathe, and becomes a
 * dropdown when they cannot. The decision belongs here rather than in the tile
 * because it is a question about the room, not about the column — and it is
 * only a default: `slicerMode` is on the edit whitelist, so a choice made in
 * the chart editor survives the next re-run.
 */
export function fitFilters(slicers = [], { charts = 0, cards = 0 } = {}) {
  if (!slicers.length) return { mode: DEFAULT_SLICER_MODE, height: 0 };

  const asList = Math.max(...slicers.map((slicer) => listHeight(slicer.values)));
  const chartRows = Math.min(MAX_CHART_ROWS, Math.max(1, Math.ceil(charts / PREFERRED_PER_ROW)));
  const spent = (cards ? KPI_HEIGHT + GUTTER : 0) + GUTTER + GUTTER * Math.max(0, chartRows - 1);
  const perRow = (height) => (CANVAS_HEIGHT - spent - height) / chartRows;

  const mode = perRow(asList) >= COMFORTABLE_ROW ? 'list' : 'dropdown';
  const height = mode === 'list' ? asList : DROPDOWN_HEIGHT;
  for (const slicer of slicers) slicer.slicerMode = slicer.slicerMode || mode;
  return { mode, height };
}
 ...
 * meant to remove.
 *
 * It also decides how the filters offer their values — see `fitFilters` — and
 * writes that onto them, because the room available is the only thing that can
 * decide it and this is the only place that knows the room.
 *
...
export function composeBoard(charts = [], slicers = [], cards = []) {
  ...
  const filters = fitFilters(rail, { charts: charts.length, cards: cards.length });
  if (rail.length) {
    rows.push({ ids: rail.map((slicer) => String(slicer.id)), height: filters.height, fixed: true });
  }
