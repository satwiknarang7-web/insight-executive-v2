...
test('a finding on a guessed column cannot call itself strong', async () => {
  const { analyzeChart } = await import('../lib/insightEngine.js');
  const { createConfidence, noteUncertain, UNCERTAIN } = await import('../lib/cellConfidence.js');

  // A pronounced, well-sampled ranking: exactly the shape that earns 'strong'.
  const chart = {
    id: 'c1',
    chart_type: 'bar',
    title: 'Revenue by region',
    dimension: 'region',
    sql: 'SELECT [region], SUM([amount]) AS [Total] FROM [d] GROUP BY [region]',
    xAxisKey: 'region',
    yAxisKey: 'Total',
    resultData: [
      { region: 'North', Total: 900 },
      { region: 'South', Total: 120 },
      { region: 'East', Total: 90 },
      { region: 'West', Total: 60 },
      { region: 'Mid', Total: 40 },
      { region: 'Far', Total: 20 },
      { region: 'Near', Total: 10 },
      { region: 'Other', Total: 5 },
    ],
  };

  const clean = analyzeChart(chart, 400, null);
  assert.ok(clean, 'the chart analyses at all');

  // Three quarters of the summed column was a coin toss.
  const store = createConfidence();
  for (let i = 0; i < 300; i++) noteUncertain(store, 'amount', i, UNCERTAIN.DECIMAL_COMMA);
  const doubted = analyzeChart(chart, 400, store);

  const rank = ['thin', 'indicative', 'moderate', 'strong'];
  assert.ok(
    rank.indexOf(doubted.metrics.evidence) < rank.indexOf(clean.metrics.evidence),
    `expected a demotion, got ${clean.metrics.evidence} -> ${doubted.metrics.evidence}`
  );
  assert.equal(doubted.metrics.evidence, 'thin', 'past half, there is no finding here');
  assert.ok(
    doubted.metrics.evidenceNotes.some((n) => n.includes('amount')),
    'the reader is told which column and why'
  );
});

test('a small doubt is reported without changing the tier', async () => {
  const { analyzeChart } = await import('../lib/insightEngine.js');
  const { createConfidence, noteUncertain, UNCERTAIN } = await import('../lib/cellConfidence.js');
  ...
  const clean = analyzeChart(chart, 400, null);
  const store = createConfidence();
  for (let i = 0; i < 4; i++) noteUncertain(store, 'amount', i, UNCERTAIN.DECIMAL_COMMA);
  const doubted = analyzeChart(chart, 400, store);

  assert.equal(doubted.metrics.evidence, clean.metrics.evidence, 'one percent changes nothing');
  assert.ok(
    doubted.metrics.evidenceNotes.length > clean.metrics.evidenceNotes.length,
